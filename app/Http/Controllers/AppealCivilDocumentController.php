...
class AppealCivilDocumentController extends Controller
{
    ...
    {
        $docs = AppealCivilDocument::where('civil_case_id', $caseId)->get();
    ...
    public function store(Request $request)
    {
        ...
        $toDelete = AppealCivilDocument::where('civil_case_id', $caseId)->whereNotIn('DID', $currentDIDs)->get();
        ...
        foreach ($docData as $index => $data) {
            ...
            if ($request->hasFile("documents.$index.file")) {
                if ($did) {
                    $existing = AppealCivilDocument::find($did);
                    if ($existing && $existing->file_path) Storage::disk('public')->delete($existing->file_path);
                }
                $fields['file_path'] = $request->file("documents.$index.file")->store('appeal_civil_uploads/Evidence_Documents', 'public');
            }

            if ($did) {
                $fields['updatedBy']   = auth()->user()->name ?? 'Admin';
                $fields['updatedDate'] = now()->format('Y-m-d');
                AppealCivilDocument::where('DID', $did)->update($fields);
            } else {
                $fields['addedBy']   = auth()->user()->name ?? 'Admin';
                $fields['addedDate'] = now()->format('Y-m-d');
                AppealCivilDocument::create($fields);
            }
        }
    ...
    {
        $doc = AppealCivilDocument::findOrFail($id);
