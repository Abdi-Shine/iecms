...
use App\Models\AppealCivilHearing;
use App\Models\AppealCivilHearingScripture;
...
class AppealCivilHearingController extends Controller
{
    public function hearingCases(Request $request)
    {
        $query = AppealCivilRegistration::with('handover', 'court', 'hearings')->orderByDesc('ACID');
        ...
        $hearings     = AppealCivilHearing::all();
        $hearingStats = [
    ...
    {
        $query = AppealCivilRegistration::with('handover', 'court')->orderByDesc('ACID');
    ...
    {
        $hearings = AppealCivilHearing::with('civilCase.court')->orderByDesc('hearing_date')->orderByDesc('hearing_time')->get();
        $cases    = AppealCivilRegistration::orderByDesc('ACID')->get(['ACID', 'FileNo', 'CaseType', 'GradeCourt']);
    ...
    {
        $hearings = AppealCivilHearing::with(['civilCase.court', 'civilCase.parties', 'civilCase.assignments.employee'])->orderBy('hearing_date')->orderBy('hearing_time')->get();
        $stats = [
            'total'     => $hearings->count(),
            'scheduled' => $hearings->where('status', 'Scheduled')->count(),
            'completed' => $hearings->where('status', 'Completed')->count(),
    ...
    {
        $hearing      = AppealCivilHearing::with('civilCase.court')->findOrFail($id);
        $cases        = AppealCivilRegistration::orderByDesc('ACID')->get(['ACID', 'FileNo', 'CaseType', 'GradeCourt']);
        $courts       = Court::orderBy('longName')->get();
        $statuses     = StatusProcess::orderBy('name')->get();
    ...
    public function store(Request $request)
    {
        ...
        $hearing = AppealCivilHearing::create($data);

        if ($isDraft) {
            return redirect()->route('appeal-hearings.edit', $hearing->id)
        ...
        AppealCivilRegistration::where('ACID', $request->input('civil_case_id'))
            ->update(['Status' => $newStatus]);

        if ($request->expectsJson()) {
            return response()->json(['success' => true, 'message' => 'Hearing scheduled successfully.']);
        }
        return redirect()->route('appeal-hearings.index')->with('success', 'Hearing scheduled successfully.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $hearing = AppealCivilHearing::findOrFail($id);
        $data    = $request->only('civil_case_id', 'hearing_date', 'hearing_time', 'duration', 'courtroom', 'hearing_purpose');
        $data['status'] = $isDraft ? 'Draft' : $request->input('status');
        $hearing->update($data);
        ...
        return redirect()->route('appeal-hearings.index')->with('success', 'Hearing updated successfully.');
    }

    public function createScripture($caseId)
    {
        $case      = AppealCivilRegistration::with('court')->findOrFail($caseId);
        $hearings  = AppealCivilHearing::where('civil_case_id', $caseId)
    ...
    public function storeScripture(Request $request)
    {
        ...
        $request->validate([
            ...
            'hearing_id'    => $isDraft ? 'nullable' : 'required|exists:appeal_civil_hearings,id',
            'hearing_date'  => 'nullable|date',
            'hearing_time'  => 'nullable|date_format:H:i',
        ]);

        $scripture = AppealCivilHearingScripture::create([
        ...
        if ($isDraft) {
            return redirect()->route('appeal-hearings.scripture.edit', $scripture->id)
                ->with('success', 'Qoraalka ayaa Draft ahaan lagu keydsaday.');
        }

        $newStatus = StatusProcess::where('name', 'Dhageysi')->value('name') ?? 'Dhageysi';
        AppealCivilRegistration::where('ACID', $scripture->civil_case_id)
            ->update(['Status' => $newStatus]);

        return redirect()->route('appeal-hearings.scripture')
            ->with('success', 'Garmaqalka si guul leh ayaa loo gudbiiyay.');
    }

    public function editScripture($id)
    {
        $scripture = AppealCivilHearingScripture::findOrFail($id);
        $case      = AppealCivilRegistration::with('court')->findOrFail($scripture->civil_case_id);
    ...
    {
        $hearing = AppealCivilHearing::with([
            'civilCase.court',
            'civilCase.parties',
        ...
        $signatures = DocumentSignature::with('signer')
            ->where('document_type', 'appeal_hearing')
    ...
    {
        $scripture = AppealCivilHearingScripture::with([
            'civilCase.court',
            'civilCase.parties',
            'civilCase.legalRepresentatives',
            'civilCase.assignments.employee',
    ...
    {
        AppealCivilHearing::findOrFail($id)->delete();
