...
use App\Models\AppealCivilJudgmentReceipt;
...
class AppealCivilJudgmentController extends Controller
{
    public function index(Request $request)
    {
        $query = AppealCivilRegistration::with([
            'court',
        ...
        $stats = [
            'total'     => AppealCivilJudgment::count(),
            'draft'     => AppealCivilJudgment::where('status', 'Draft')->count(),
    ...
    public function store(Request $request)
    {
        ...
        $judgment = AppealCivilJudgment::create([
            ...$request->only('civil_case_id', 'judgment_date', 'judgment_time', 'judgment_type'),
        ...
        if (!$isDraft) {
            ...
            AppealCivilRegistration::where('ACID', $request->civil_case_id)
                ->update(['Status' => $newStatus]);
        }

        if ($isDraft) {
            return redirect()->route('appeal-judgments.edit', $judgment->id)
                ->with('success', 'Xukunku Draft ahaan ayaa lagu keydsaday.');
        }
        return redirect()->route('appeal-judgments.index')
    ...
    {
        $judgment = AppealCivilJudgment::with('receipts')->findOrFail($id);
        $case     = AppealCivilRegistration::with('court', 'parties', 'legalRepresentatives.party', 'assignments.employee')
                        ->findOrFail($judgment->civil_case_id);
        $court    = $case->court;
    ...
    public function receiptsIndex(Request $request)
    {
        ...
        $stats = [
            'total'          => AppealCivilJudgment::count(),
            'fully_received' => AppealCivilJudgment::whereHas('receipts', fn($q) => $q->groupBy('judgment_id'))->count(),
            'partial'        => 0,
    ...
    {
        $judgment = AppealCivilJudgment::with('civilCase.parties')->findOrFail($judgmentId);
        $party    = $judgment->civilCase->parties->where('PID', $partyId)->firstOrFail();

        $signaturePath = AppealCivilJudgmentReceipt::where('judgment_id', $judgmentId)
        ...
        if ($request->filled('signature_data')) {
            ...
            if ($decoded) {
                $filename = 'appeal_civil_uploads/judgment-signatures/' . uniqid('sig_', true) . '.png';
                Storage::disk('public')->put($filename, $decoded);
                $signaturePath = $filename;
            }
        }

        $repSignaturePath = AppealCivilJudgmentReceipt::where('judgment_id', $judgmentId)
        ...
        if ($request->filled('rep_signature_data')) {
            ...
            $decoded = base64_decode($base64);
            if ($decoded) {
                $filename = 'appeal_civil_uploads/judgment-signatures/' . uniqid('rep_', true) . '.png';
                Storage::disk('public')->put($filename, $decoded);
                $repSignaturePath = $filename;
            }
        }

        AppealCivilJudgmentReceipt::updateOrCreate(
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$isDraft) {
            ...
            AppealCivilRegistration::where('ACID', $judgment->civil_case_id)
                ->update(['Status' => $newStatus]);
        }
