...
use App\Models\AppealCriminalAssignment;
...
class AppealCriminalAssignmentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = (clone $baseQuery)->with(['assignments.employee', 'court'])->orderByDesc('ACMID');
        ...
        $assignments = AppealCriminalAssignment::with(['case.court', 'employee'])->latest()->get();
        $employees   = Employee::where('status', 'Active')
                               ->when($courtId, fn($q) => $q->where('courtID', $courtId))
                               ->get();

        return view('appeal_court.Appeal_criminal.assign.appeal_criminal_assign_view', compact('cases', 'assignments', 'employees', 'stats'));
    }
    ...
    public function addJudges($id)
    {
        ...
        return view('appeal_court.Appeal_criminal.assign.appeal_criminal_add_judges', compact('case', 'employees', 'stageStatus'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $case        = AppealCriminalRegistration::findOrFail($request->input('criminal_case_id'));
        $stageStatus = StatusProcess::where('name', 'Gal Ku Qoris')->first();
        $newStatus   = $stageStatus?->name ?? 'Gal Ku Qoris';

        AppealCriminalAssignment::create([
    ...
    {
        $assignment = AppealCriminalAssignment::findOrFail($id);
        $assignment->update([
            'employee_id'     => $request->input('employee_id'),
            'panel_role'      => $request->input('panel_role'),
            'assignment_date' => $request->input('assignment_date'),
    ...
    {
        AppealCriminalAssignment::findOrFail($id)->delete();
