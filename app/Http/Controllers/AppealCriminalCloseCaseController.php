...
use App\Models\AppealCriminalCloseCase;
...
class AppealCriminalCloseCaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        $stats = [
            ...
            'xukun'  => AppealCriminalRegistration::where('Status', 'Xukun')->count(),
            'closed' => AppealCriminalRegistration::where('Status', 'Closed')->count(),
            'final'  => AppealCriminalJudgment::where('status', 'Final')->count(),
        ];

        return view('appeal_court.Appeal_criminal.Conclusion.appeal_criminal_view_Close', compact('records', 'statuses', 'stats'));
    }

    public function document($caseId)
    {
        $case      = AppealCriminalRegistration::with(['court', 'parties', 'assignments.employee'])->findOrFail($caseId);
        $closeCase = AppealCriminalCloseCase::where('criminal_case_id', $caseId)->latest()->firstOrFail();
        $court     = $case->court;
        $judgment  = $closeCase;
        ...
        $signatures = DocumentSignature::with('signer')
            ->where('document_type', 'appeal_criminal_close_case')
        ...
        $stampSigs        = DocumentSignature::where('document_type', 'appeal_criminal_close_case_stamp')
        ...
        return view('appeal_court.Appeal_criminal.Conclusion.appeal_criminal_document_Close',
    ...
    public function form($caseId)
    {
        ...
        $closeCase  = AppealCriminalCloseCase::where('criminal_case_id', $caseId)->latest()->first();
        ...
        return view('appeal_court.Appeal_criminal.Conclusion.appeal_criminal_add_Close',
    ...
    public function store(Request $request)
    {
        ...
        AppealCriminalCloseCase::updateOrCreate(
        ...
        if (!$isDraft) {
            AppealCriminalRegistration::where('ACMID', $caseId)->update(['Status' => 'Oodista']);
        }

        return $isDraft
            ? redirect()->route('appeal-criminal-close-case.form', $caseId)->with('success', 'Muswaadda si guul leh ayaa lagu keydsaday.')
            : redirect()->route('appeal-criminal-close-case.index')->with('success', 'Kiiska xidhitaankiisu si guul leh ayaa loo gudbiyay.');
    }
