...
class AppealCriminalDocumentController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('appeal_court.Appeal_criminal.registration.appeal_criminal_document_support', compact('case', 'docTypes'));
    }

    public function getDocumentsByCase($caseId)
    {
        $docs = AppealCriminalDocument::where('criminal_case_id', $caseId)->get();
    ...
    public function store(Request $request)
    {
        ...
        $toDelete = AppealCriminalDocument::where('criminal_case_id', $caseId)->whereNotIn('DID', $currentDIDs)->get();
        ...
        foreach ($docData as $index => $data) {
            ...
            if ($request->hasFile("documents.$index.file")) {
                if ($did) {
                    $existing = AppealCriminalDocument::find($did);
                    if ($existing && $existing->file_path) Storage::disk('public')->delete($existing->file_path);
                }
                $fields['file_path'] = $request->file("documents.$index.file")->store('appeal_criminal_uploads/Evidence_Documents', 'public');
            }

            if ($did) {
                $fields['updatedBy']   = auth()->user()->name ?? 'Admin';
                $fields['updatedDate'] = now()->format('Y-m-d');
                AppealCriminalDocument::where('DID', $did)->update($fields);
            } else {
                $fields['addedBy']   = auth()->user()->name ?? 'Admin';
                $fields['addedDate'] = now()->format('Y-m-d');
                AppealCriminalDocument::create($fields);
            }
        }
    ...
    {
        $doc = AppealCriminalDocument::findOrFail($id);
