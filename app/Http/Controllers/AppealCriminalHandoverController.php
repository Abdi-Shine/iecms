...
use App\Models\AppealCriminalDocument;
use App\Models\AppealCriminalHandover;
...
class AppealCriminalHandoverController extends Controller
{
    public function create($id)
    {
        ...
        $existing = AppealCriminalHandover::where('criminal_case_id', $id)->latest()->first();

        $caseDocuments = AppealCriminalDocument::where('criminal_case_id', $id)
            ->get()
            ->map(function ($doc) {
                $pages = 0;
                if ($doc->file_path) {
        ...
        $existingReturnFile  = AppealCriminalReturnFile::where('criminal_case_id', $id)->latest()->first();
        $returnFileDocuments = $existingReturnFile?->documents ?? $caseDocuments->toArray();

        return view('appeal_court.Appeal_criminal.Conclusion.appeal_criminal_add_handover',
    ...
    public function document($id)
    {
        ...
        return view('appeal_court.Appeal_criminal.Conclusion.appeal_criminal_handover_document', $data);
    }
    ...
    public function documentPdf($id)
    {
        ...
        return \App\Support\CourtDocumentPdf::stream(
            'appeal_court.Appeal_criminal.Conclusion.appeal_criminal_handover_document',
            $data,
    ...
    {
        $case     = AppealCriminalRegistration::with(['court', 'assignments.employee'])->findOrFail($id);
        $handover = AppealCriminalHandover::where('criminal_case_id', $id)->latest()->first();
        ...
        $signatures = $handover
            ? DocumentSignature::with('signer')
                ->where('document_type', 'appeal_criminal_handover')
    ...
    public function store(Request $request)
    {
        ...
        $wasSubmitted = AppealCriminalHandover::where('criminal_case_id', $caseId)->value('status') === 'Sug Qaatay';

        $handover = AppealCriminalHandover::updateOrCreate(
    ...
    private function notifyHandoverApprovers(AppealCriminalHandover $handover, $caseId): void
    {
        try {
            $case = AppealCriminalRegistration::find($caseId);
            if (!$case) return;
    ...
    {
        $baseQuery = AppealCriminalHandover::query();
        ...
        $caseTypes = AppealCriminalRegistration::whereNotNull('CaseType')
            ->whereIn('ACMID', (clone $baseQuery)->pluck('criminal_case_id'))
            ->distinct()
            ->orderBy('CaseType')
            ->pluck('CaseType');
        ...
        return view('appeal_court.Appeal_criminal.Conclusion.appeal_criminal_handover_approval', compact('handovers', 'pending', 'approved', 'rejected', 'draft', 'caseTypes'));
    }
     ...
     * the "Saxiix Dukuumintiga" button (document_type=appeal_criminal_handover, role=clerk).
     */
    public function approve(Request $request, $id)
    {
        ...
        $handover = AppealCriminalHandover::where('criminal_case_id', $id)->latest()->firstOrFail();

        \App\Models\DocumentSignature::updateOrCreate(
            [
                'document_type' => 'appeal_criminal_handover',
            ...
            [
                ...
                'content_hash' => hash('sha256', 'appeal_criminal_handover|' . $handover->id . '|' . $handover->criminal_case_id . '|' . $newStatus),
            ]
        );

        $handover->update(['status' => $newStatus]);
        AppealCriminalRegistration::where('ACMID', $id)->update(['Status' => $newStatus]);
