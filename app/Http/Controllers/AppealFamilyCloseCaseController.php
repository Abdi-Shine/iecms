...
use App\Models\AppealFamilyCloseCase;
...
class AppealFamilyCloseCaseController extends Controller
{
    public function index(Request $request)
    {
        ...
        $stats = [
            ...
            'xukun'  => AppealFamilyRegistration::where('Status', 'Xukun')->count(),
            'closed' => AppealFamilyRegistration::where('Status', 'Closed')->count(),
            'final'  => AppealFamilyJudgment::where('status', 'Final')->count(),
        ];

        return view('appeal_court.Appeal_family.Conclusion.appeal_family_view_Close', compact('records', 'statuses', 'stats'));
    }

    public function document($caseId)
    {
        $case      = AppealFamilyRegistration::with(['court', 'parties', 'assignments.employee'])->findOrFail($caseId);
        $closeCase = AppealFamilyCloseCase::where('family_case_id', $caseId)->latest()->firstOrFail();
        $court     = $case->court;
        $judgment  = $closeCase;
        ...
        $signatures = DocumentSignature::with('signer')
            ->where('document_type', 'appeal_family_close_case')
        ...
        $stampSigs        = DocumentSignature::where('document_type', 'appeal_family_close_case_stamp')
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_document_Close',
    ...
    public function form($caseId)
    {
        ...
        $closeCase  = AppealFamilyCloseCase::where('family_case_id', $caseId)->latest()->first();
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_add_Close',
    ...
    public function store(Request $request)
    {
        ...
        AppealFamilyCloseCase::updateOrCreate(
        ...
        if (!$isDraft) {
            AppealFamilyRegistration::where('AFCID', $caseId)->update(['Status' => 'Oodista']);
        }

        return $isDraft
            ? redirect()->route('appeal-family-close-case.form', $caseId)->with('success', 'Muswaadda si guul leh ayaa lagu keydsaday.')
            : redirect()->route('appeal-family-close-case.index')->with('success', 'Kiiska xidhitaankiisu si guul leh ayaa loo gudbiyay.');
    }
