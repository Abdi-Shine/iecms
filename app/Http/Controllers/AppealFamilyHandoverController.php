...
use App\Models\AppealFamilyHandover;
use App\Models\AppealFamilyReturnFile;
...
class AppealFamilyHandoverController extends Controller
{
    public function create($id)
    {
        ...
        $existing = AppealFamilyHandover::where('family_case_id', $id)->latest()->first();

        $caseDocuments = AppealFamilyDocument::where('family_case_id', $id)
            ->get()
            ->map(function ($doc) {
                $pages = 0;
                if ($doc->file_path) {
        ...
        $existingReturnFile  = AppealFamilyReturnFile::where('family_case_id', $id)->latest()->first();
        $returnFileDocuments = $existingReturnFile?->documents ?? $caseDocuments->toArray();

        return view('appeal_court.Appeal_family.Conclusion.appeal_family_add_handover',
    ...
    public function document($id)
    {
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_handover_document', $data);
    }
    ...
    public function documentPdf($id)
    {
        ...
        return \App\Support\CourtDocumentPdf::stream(
            'appeal_court.Appeal_family.Conclusion.appeal_family_handover_document',
            $data,
    ...
    {
        $case     = AppealFamilyRegistration::with(['court', 'assignments.employee'])->findOrFail($id);
        $handover = AppealFamilyHandover::where('family_case_id', $id)->latest()->first();
        ...
        $signatures = $handover
            ? DocumentSignature::with('signer')
                ->where('document_type', 'appeal_family_handover')
    ...
    public function store(Request $request)
    {
        ...
        $wasSubmitted = AppealFamilyHandover::where('family_case_id', $caseId)->value('status') === 'Sug Qaatay';

        $handover = AppealFamilyHandover::updateOrCreate(
    ...
    private function notifyHandoverApprovers(AppealFamilyHandover $handover, $caseId): void
    {
        try {
            $case = AppealFamilyRegistration::find($caseId);
            if (!$case) return;
    ...
    {
        $baseQuery = AppealFamilyHandover::query();
        ...
        $caseTypes = AppealFamilyRegistration::whereNotNull('CaseType')
            ->whereIn('AFCID', (clone $baseQuery)->pluck('family_case_id'))
            ->distinct()
            ->orderBy('CaseType')
            ->pluck('CaseType');
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_handover_approval', compact('handovers', 'pending', 'approved', 'rejected', 'draft', 'caseTypes'));
    }
     ...
     * the "Saxiix Dukuumintiga" button (document_type=appeal_family_handover, role=clerk).
     */
    public function approve(Request $request, $id)
    {
        ...
        $handover = AppealFamilyHandover::where('family_case_id', $id)->latest()->firstOrFail();

        \App\Models\DocumentSignature::updateOrCreate(
            [
                'document_type' => 'appeal_family_handover',
            ...
            [
                ...
                'content_hash' => hash('sha256', 'appeal_family_handover|' . $handover->id . '|' . $handover->family_case_id . '|' . $newStatus),
            ]
        );

        $handover->update(['status' => $newStatus]);
        AppealFamilyRegistration::where('AFCID', $id)->update(['Status' => $newStatus]);
