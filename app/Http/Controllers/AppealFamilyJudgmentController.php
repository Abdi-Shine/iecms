...
use App\Models\AppealFamilyJudgment;
use App\Models\AppealFamilyJudgmentReceipt;
...
class AppealFamilyJudgmentController extends Controller
{
    public function index(Request $request)
    {
        $query = AppealFamilyRegistration::with([
            'court',
        ...
        $stats = [
            'total'     => AppealFamilyJudgment::count(),
            'draft'     => AppealFamilyJudgment::where('status', 'Draft')->count(),
            'submitted' => AppealFamilyJudgment::where('status', 'Submitted')->count(),
            'final'     => AppealFamilyJudgment::where('status', 'Final')->count(),
        ];
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_view_Judgment', compact('records', 'statuses', 'stats'));
    }
    ...
    public function create($caseId)
    {
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_add_Judgment', compact('case', 'judgment', 'judge'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $judgment = AppealFamilyJudgment::create([
        ...
        if (!$isDraft) {
            ...
            AppealFamilyRegistration::where('AFCID', $request->family_case_id)
                ->update(['Status' => $newStatus]);
        }

        if ($isDraft) {
            return redirect()->route('appeal-family-judgments.edit', $judgment->id)
                ->with('success', 'Xukunku Draft ahaan ayaa lagu keydsaday.');
        }
        return redirect()->route('appeal-family-judgments.index')
            ->with('success', 'Xukunku si guul leh ayaa loo gudbiiyay.');
    }

    public function edit($id)
    {
        $judgment = AppealFamilyJudgment::findOrFail($id);
        $case     = AppealFamilyRegistration::with('court', 'assignments.employee')->findOrFail($judgment->family_case_id);
        $judge    = $case->assignments->whereIn('panel_role', ['Chair', 'Guddoomiye'])->first()?->employee?->EmpName ?? '—';
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_add_Judgment', compact('case', 'judgment', 'judge'));
    }
    ...
    {
        $judgment = AppealFamilyJudgment::with('receipts')->findOrFail($id);
        $case     = AppealFamilyRegistration::with('court', 'parties', 'legalRepresentatives.party', 'assignments.employee')
        ...
        $signatures = DocumentSignature::with('signer')
            ->where('document_type', 'appeal_family_judgment')
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_document_Judgment',
    ...
    public function receiptsIndex(Request $request)
    {
        ...
        $stats = [
            'total'          => AppealFamilyJudgment::count(),
            'fully_received' => AppealFamilyJudgment::whereHas('receipts', fn($q) => $q->groupBy('judgment_id'))->count(),
            'partial'        => 0,
            'not_received'   => AppealFamilyJudgment::doesntHave('receipts')->count(),
        ];

        return view('appeal_court.Appeal_family.Conclusion.appeal_family_view_Judgment_taking_parties',
            compact('records', 'statuses', 'stats'));
    }

    public function confirmReceipt(Request $request, $judgmentId, $partyId)
    {
        $judgment = AppealFamilyJudgment::with('familyCase.parties')->findOrFail($judgmentId);
        $party    = $judgment->familyCase->parties->where('PID', $partyId)->firstOrFail();

        $signaturePath = AppealFamilyJudgmentReceipt::where('judgment_id', $judgmentId)
        ...
        if ($request->filled('signature_data')) {
            ...
            if ($decoded) {
                $filename = 'appeal_family_uploads/judgment-signatures/' . uniqid('sig_', true) . '.png';
                Storage::disk('public')->put($filename, $decoded);
                $signaturePath = $filename;
            }
        }

        $repSignaturePath = AppealFamilyJudgmentReceipt::where('judgment_id', $judgmentId)
        ...
        if ($request->filled('rep_signature_data')) {
            ...
            $decoded = base64_decode($base64);
            if ($decoded) {
                $filename = 'appeal_family_uploads/judgment-signatures/' . uniqid('rep_', true) . '.png';
                Storage::disk('public')->put($filename, $decoded);
                $repSignaturePath = $filename;
            }
        }

        AppealFamilyJudgmentReceipt::updateOrCreate(
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$isDraft) {
            ...
            AppealFamilyRegistration::where('AFCID', $judgment->family_case_id)
                ->update(['Status' => $newStatus]);
        }
