...
use App\Models\AppealFamilyLegalRepresentative;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class AppealFamilyLegalRepresentativeController extends Controller
{
    public function getByCase($caseId)
    {
        $reps = AppealFamilyLegalRepresentative::with('party')
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('rep_doc')) {
            if ($id) {
                $existing = AppealFamilyLegalRepresentative::find($id);
                if ($existing?->rep_doc) Storage::disk('public')->delete($existing->rep_doc);
            }
            $fields['rep_doc'] = $request->file('rep_doc')->store('appeal_family_uploads/legal_reps', 'public');
        }

        if ($id) {
            $fields['addedBy'] = auth()->user()->name ?? 'Admin';
            AppealFamilyLegalRepresentative::where('id', $id)->update($fields);
            $rep = AppealFamilyLegalRepresentative::find($id);
        } else {
            $fields['addedBy']   = auth()->user()->name ?? 'Admin';
            $fields['addedDate'] = now()->toDateString();
            $rep = AppealFamilyLegalRepresentative::create($fields);
        }
    ...
    {
        $rep = AppealFamilyLegalRepresentative::findOrFail($id);
