...
use App\Models\AppealFamilyParty;
...
class AppealFamilyPartyController extends Controller
{
    public function index(Request $request)
    {
        $caseId = $request->query('case_id');
        if (!$caseId) {
            return redirect()->route('appeal-family-registration.index')->with('error', 'Case ID is required.');
        }
        $case    = AppealFamilyRegistration::findOrFail($caseId);
        $lawyers = Lawyer::where('status', 'active')->orderBy('LawyerName')->get();
        return view('appeal_court.Appeal_family.registration.appeal_family_Parties_information', compact('case', 'lawyers'));
    }

    public function getPartiesByCase($caseId)
    {
        $parties = AppealFamilyParty::where('family_case_id', $caseId)->get();
    ...
    {
        $request->validate([
            'case_id' => 'required|exists:appeal_family_registrations,AFCID',
        ...
        AppealFamilyParty::where('family_case_id', $caseId)->whereNotIn('PID', $currentPIDs)->delete();
        ...
        foreach ($partyData as $index => $data) {
            ...
            if ($request->hasFile("parties.$index.passport_doc")) {
                $fields['passport_doc'] = $request->file("parties.$index.passport_doc")->store('appeal_family_uploads/Parties_documents', 'public');
            }
            if ($request->hasFile("parties.$index.power_of_attorney")) {
                $fields['power_of_attorney'] = $request->file("parties.$index.power_of_attorney")->store('appeal_family_uploads/Parties_documents', 'public');
            }

            if ($pid) {
                $fields['updatedBy']   = auth()->user()->name ?? 'Admin';
                $fields['updatedDate'] = now()->format('Y-m-d');
                AppealFamilyParty::where('PID', $pid)->update($fields);
            } else {
                $fields['addedBy']   = auth()->user()->name ?? 'Admin';
                $fields['addedDate'] = now()->format('Y-m-d');
                AppealFamilyParty::create($fields);
            }
        }
    ...
    {
        $party = AppealFamilyParty::findOrFail($id);
    ...
    public function sendNotifications($caseId)
    {
        ...
        $parties = AppealFamilyParty::where('family_case_id', $caseId)->where('party_role', 'Plaintiff')->whereNotNull('email')->where('email', '!=', '')->get();
