...
class AppealFamilyReturnFileController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = (clone $baseQuery)->with(['court', 'returnFile', 'judgments' => fn($q) => $q->with('receipts')->orderByDesc('created_at')->limit(1)])->orderByDesc('AFCID');
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_view_return_file',
            compact('records', 'statuses', 'stats'));
    }

    public function create($id)
    {
        $case  = AppealFamilyRegistration::with(['court', 'assignments.employee', 'documents'])->findOrFail($id);
        ...
        $existing = AppealFamilyReturnFile::where('family_case_id', $id)->latest()->first();
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_add_return_file',
    ...
    public function document($id)
    {
        ...
        return view('appeal_court.Appeal_family.Conclusion.appeal_family_document_return_file', $data);
    }
    ...
    public function documentPdf($id)
    {
        ...
        return \App\Support\CourtDocumentPdf::stream(
            'appeal_court.Appeal_family.Conclusion.appeal_family_document_return_file',
            $data,
            'Appeal-Family-ReturnFile-' . $data['case']->FileNo . '.pdf'
        );
    }
    ...
    public function store(Request $request)
    {
        ...
        AppealFamilyReturnFile::updateOrCreate(
        ...
        if ($status === 'Qaatay') {
            ...
            AppealFamilyRegistration::where('AFCID', $caseId)->update(['Status' => $newStatus]);
        }
