...
use App\Models\CivilLegalRepresentative;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class CivilLegalRepresentativeController extends Controller
{
    public function getByCase($caseId)
    {
        $reps = CivilLegalRepresentative::with('party')
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('rep_doc')) {
            if ($id) {
                $existing = CivilLegalRepresentative::find($id);
                if ($existing?->rep_doc) Storage::disk('public')->delete($existing->rep_doc);
            }
            $fields['rep_doc'] = $request->file('rep_doc')->store('district_civil_uploads/legal_reps', 'public');
        }

        if ($id) {
            $fields['addedBy']   = auth()->user()->name ?? 'Admin';
            CivilLegalRepresentative::where('id', $id)->update($fields);
            $rep = CivilLegalRepresentative::find($id);
        } else {
            $fields['addedBy']   = auth()->user()->name ?? 'Admin';
            $fields['addedDate'] = now()->toDateString();
            $rep = CivilLegalRepresentative::create($fields);
        }
    ...
    {
        $rep = CivilLegalRepresentative::findOrFail($id);
