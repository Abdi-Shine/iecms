...
class DistrictCriminalHandoverController extends Controller
{
    public function create($id)
    {
        ...
        $existing = DistrictCriminalHandover::where('criminal_case_id', $id)->latest()->first();

        $caseDocuments = DistrictCriminalDocument::where('criminal_case_id', $id)
            ->get()
            ->map(function ($doc) {
                $pages = 0;
                if ($doc->file_path) {
    ...
    {
        $case     = DistrictCriminalRegistration::with(['court', 'assignments.employee'])->findOrFail($id);
        $handover = DistrictCriminalHandover::where('criminal_case_id', $id)->latest()->first();
        ...
        $signatures = $handover
            ? DocumentSignature::with('signer')
                ->where('document_type', 'criminal_handover')
    ...
    public function store(Request $request)
    {
        ...
        $wasSubmitted = DistrictCriminalHandover::where('criminal_case_id', $caseId)->value('status') === 'Sug Qaatay';

        $handover = DistrictCriminalHandover::updateOrCreate(
    ...
    private function notifyHandoverApprovers(DistrictCriminalHandover $handover, $caseId): void
    {
        try {
            $case = DistrictCriminalRegistration::find($caseId);
            if (!$case) return;

            $approvers = \App\Models\User::whereHas('group.roles.permissions', function ($q) {
                $q->where('module', 'Criminal Case Handover Approval')->where('action', 'view');
            })->orWhereNull('group_id')->get();

            foreach ($approvers as $user) {
                $user->notify(new \App\Notifications\CriminalHandoverSubmittedNotification(
                    $handover,
                    $case->FileNo,
            ...
            \Log::error('CriminalHandoverSubmittedNotification failed: ' . $e->getMessage());
        }
    }

    public function approvalIndex(Request $request)
    {
        $baseQuery = DistrictCriminalHandover::query();
        ...
        $caseTypes = DistrictCriminalRegistration::whereNotNull('CaseType')
            ->whereIn('CMID', (clone $baseQuery)->pluck('criminal_case_id'))
            ->distinct()
            ->orderBy('CaseType')
            ->pluck('CaseType');
     ...
     * document view page (document_type=criminal_handover, role=clerk).
     */
    public function approve(Request $request, $id)
    {
        ...
        $handover = DistrictCriminalHandover::where('criminal_case_id', $id)->latest()->firstOrFail();

        DocumentSignature::updateOrCreate(
            [
                'document_type' => 'criminal_handover',
            ...
            [
                ...
                'content_hash' => hash('sha256', 'criminal_handover|' . $handover->id . '|' . $handover->criminal_case_id . '|' . $newStatus),
            ]
        );

        $handover->update(['status' => $newStatus]);
        DistrictCriminalRegistration::where('CMID', $id)->update(['Status' => $newStatus]);
