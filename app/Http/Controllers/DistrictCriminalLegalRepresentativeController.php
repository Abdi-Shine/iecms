...
use App\Models\DistrictCriminalLegalRepresentative;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Storage;

class DistrictCriminalLegalRepresentativeController extends Controller
{
    public function getByCase($caseId)
    {
        $reps = DistrictCriminalLegalRepresentative::with('party')
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('rep_doc')) {
            if ($id) {
                $existing = DistrictCriminalLegalRepresentative::find($id);
                if ($existing?->rep_doc) Storage::disk('public')->delete($existing->rep_doc);
            }
            $fields['rep_doc'] = $request->file('rep_doc')->store('district_criminal_uploads/legal_reps', 'public');
        }

        if ($id) {
            $fields['addedBy']   = auth()->user()->name ?? 'Admin';
            DistrictCriminalLegalRepresentative::where('id', $id)->update($fields);
            $rep = DistrictCriminalLegalRepresentative::find($id);
        } else {
            $fields['addedBy']   = auth()->user()->name ?? 'Admin';
            $fields['addedDate'] = now()->toDateString();
            $rep = DistrictCriminalLegalRepresentative::create($fields);
        }
    ...
    {
        $rep = DistrictCriminalLegalRepresentative::findOrFail($id);
