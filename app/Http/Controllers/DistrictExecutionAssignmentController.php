...
use App\Models\DistrictExecutionAssignment;
...
class DistrictExecutionAssignmentController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = (clone $baseQuery)->with(['assignments.employee', 'court'])->orderByDesc('ECID');
        ...
        $assignments    = DistrictExecutionAssignment::with(['case.court', 'employee'])->latest()->get();
    ...
    public function store(Request $request)
    {
        ...
        $case        = DistrictExecutionRegistration::findOrFail($request->input('execution_case_id'));
        $stageStatus = StatusProcess::where('name', 'Gal Ku Qoris')->first();
        $newStatus   = $stageStatus?->name ?? 'Gal Ku Qoris';

        DistrictExecutionAssignment::create([
            'execution_case_id'  => $request->input('execution_case_id'),
    ...
    {
        $assignment = DistrictExecutionAssignment::findOrFail($id);
        $assignment->update([
            'employee_id'     => $request->input('employee_id'),
            'panel_role'      => $request->input('panel_role'),
            'assignment_date' => $request->input('assignment_date'),
    ...
    {
        DistrictExecutionAssignment::findOrFail($id)->delete();
