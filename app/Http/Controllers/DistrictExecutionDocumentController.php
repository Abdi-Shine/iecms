...
class DistrictExecutionDocumentController extends Controller
{
    ...
    {
        $docs = DistrictExecutionDocument::where('execution_case_id', $caseId)->get();
    ...
    public function store(Request $request)
    {
        ...
        $toDelete = DistrictExecutionDocument::where('execution_case_id', $caseId)->whereNotIn('DID', $currentDIDs)->get();
        ...
        foreach ($docData as $index => $data) {
            ...
            if ($request->hasFile("documents.$index.file")) {
                if ($did) {
                    $existing = DistrictExecutionDocument::find($did);
                    if ($existing && $existing->file_path) Storage::disk('public')->delete($existing->file_path);
                }
                $fields['file_path'] = $request->file("documents.$index.file")->store('district_execution_uploads/Evidence_Documents', 'public');
            }

            if ($did) {
                $fields['updatedBy']   = auth()->user()->name ?? 'Admin';
                $fields['updatedDate'] = now()->format('Y-m-d');
                DistrictExecutionDocument::where('DID', $did)->update($fields);
            } else {
                $fields['addedBy']   = auth()->user()->name ?? 'Admin';
                $fields['addedDate'] = now()->format('Y-m-d');
                DistrictExecutionDocument::create($fields);
            }
        }
    ...
    {
        $doc = DistrictExecutionDocument::findOrFail($id);
