...
use App\Models\DistrictExecutionJudgmentReceipt;
...
class DistrictExecutionJudgmentController extends Controller
{
    public function index(Request $request)
    {
        $query = DistrictExecutionRegistration::with([
            'court',
        ...
        $stats = [
            'total'     => DistrictExecutionJudgment::count(),
            'draft'     => DistrictExecutionJudgment::where('status', 'Draft')->count(),
            'submitted' => DistrictExecutionJudgment::where('status', 'Submitted')->count(),
    ...
    public function store(Request $request)
    {
        ...
        $judgment = DistrictExecutionJudgment::create([
            ...$request->only('execution_case_id', 'judgment_date', 'judgment_time', 'judgment_type'),
        ...
        if (!$isDraft) {
            ...
            DistrictExecutionRegistration::where('ECID', $request->execution_case_id)
                ->update(['Status' => $newStatus]);
        }

        if ($isDraft) {
            return redirect()->route('execution-judgments.edit', $judgment->id)
                ->with('success', 'Xukunku Draft ahaan ayaa lagu keydsaday.');
        }
        return redirect()->route('execution-judgments.index')
    ...
    {
        $judgment = DistrictExecutionJudgment::with('receipts')->findOrFail($id);
        $case     = DistrictExecutionRegistration::with('court', 'parties', 'lawyers.lawyer', 'legalRepresentatives.party', 'assignments.employee')
                        ->findOrFail($judgment->execution_case_id);
        $court    = $case->court;
        ...
        $signatures = DocumentSignature::with('signer')
            ->where('document_type', 'execution_judgment')
        ...
        $stampSigs       = JudgmentDocumentSignature::where('document_type', 'execution_judgment_stamp')
    ...
    public function receiptsIndex(Request $request)
    {
        ...
        $stats = [
            'total'           => DistrictExecutionJudgment::count(),
            'fully_received'  => DistrictExecutionJudgment::whereHas('receipts', fn($q) => $q->groupBy('judgment_id'))->count(),
            'partial'         => 0,
            'not_received'    => DistrictExecutionJudgment::doesntHave('receipts')->count(),
        ];
    ...
    {
        $judgment = DistrictExecutionJudgment::with('executionCase.parties')->findOrFail($judgmentId);
        $party    = $judgment->executionCase->parties->where('PID', $partyId)->firstOrFail();

        $signaturePath = DistrictExecutionJudgmentReceipt::where('judgment_id', $judgmentId)
        ...
        if ($request->filled('signature_data')) {
            ...
            if ($decoded) {
                $filename = 'district_execution_uploads/judgment-signatures/' . uniqid('sig_', true) . '.png';
        ...
        $repSignaturePath = DistrictExecutionJudgmentReceipt::where('judgment_id', $judgmentId)
        ...
        if ($request->filled('rep_signature_data')) {
            ...
            $decoded = base64_decode($base64);
            if ($decoded) {
                $filename = 'district_execution_uploads/judgment-signatures/' . uniqid('rep_', true) . '.png';
                Storage::disk('public')->put($filename, $decoded);
                $repSignaturePath = $filename;
            }
        }

        DistrictExecutionJudgmentReceipt::updateOrCreate(
    ...
    {
        $judgment = DistrictExecutionJudgment::with([
            'executionCase.court',
            'executionCase.parties',
            'executionCase.assignments.employee',
        ])->findOrFail($id);

        $case  = $judgment->executionCase;
        $clerk = $case->assignments->whereIn('panel_role', ['Clerk', 'Kaaliye'])->first();

        $signatures = JudgmentDocumentSignature::with('signer')
            ->where('document_type', 'execution_judgment_stamp')
            ->where('document_id', $judgment->id)
            ->get()->keyBy('role');
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$isDraft) {
            ...
            DistrictExecutionRegistration::where('ECID', $judgment->execution_case_id)
                ->update(['Status' => $newStatus]);
        }
