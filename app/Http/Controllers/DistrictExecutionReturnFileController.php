...
use App\Models\DistrictExecutionReturnFile;
...
class DistrictExecutionReturnFileController extends Controller
{
    public function index(Request $request)
    {
        $baseQuery = DistrictExecutionRegistration::query();
        ...
        $query = (clone $baseQuery)->with(['court', 'returnFile', 'judgments' => fn($q) => $q->with('receipts')->orderByDesc('created_at')->limit(1)])->orderByDesc('ECID');
        ...
        return view('distract_courts.District_execution.Conclusion.district_execution_view_return_file',
            compact('records', 'statuses', 'stats', 'executionSubCases'));
    }

    public function create($id)
    {
        $case = DistrictExecutionRegistration::with(['court', 'assignments.employee', 'documents'])->findOrFail($id);
        ...
        $existing = DistrictExecutionReturnFile::where('execution_case_id', $id)->latest()->first();
        ...
        return view('distract_courts.District_execution.Conclusion.district_execution_add_return_file',
    ...
    public function document($id)
    {
        ...
        return view('distract_courts.District_execution.Conclusion.district_execution_document_return_file', $data);
    }
    ...
    public function documentReadOnly($id)
    {
        ...
        return view('distract_courts.District_execution.Conclusion.district_execution_document_return_file_readonly', $data);
    }
    ...
    public function documentPdf($id)
    {
        ...
        return \App\Support\CourtDocumentPdf::stream(
            'distract_courts.District_execution.Conclusion.district_execution_document_return_file',
            $data,
    ...
    private function returnFileDocumentData($id): array
    {
        ...
        $returnFile = DistrictExecutionReturnFile::where('execution_case_id', $id)->latest()->first();
        $clerk      = $case->assignments->first(fn($a) => in_array($a->panel_role, ['Kaaliye', 'Clerk']));

        $signatures = $returnFile
            ? DocumentSignature::with('signer')
                ->where('document_type', 'execution_return_file')
    ...
    public function store(Request $request)
    {
        ...
        DistrictExecutionReturnFile::updateOrCreate(
        ...
        if ($status === 'Qaatay') {
            ...
            DistrictExecutionRegistration::where('ECID', $caseId)->update(['Status' => $newStatus]);
        }
