...
class DistrictFamilyHandoverController extends Controller
{
    public function create($id)
    {
        ...
        $existing = DistrictFamilyHandover::where('family_case_id', $id)->latest()->first();

        $caseDocuments = DistrictFamilyDocument::where('family_case_id', $id)
            ->get()
            ->map(function ($doc) {
                $pages = 0;
                if ($doc->file_path) {
        ...
        return view('distract_courts.District_family.registration.district_family_add_handover',
    ...
    public function document($id)
    {
        ...
        return view('distract_courts.District_family.registration.district_family_handover_document', $data);
    }
    ...
    public function documentPdf($id)
    {
        ...
        return \App\Support\CourtDocumentPdf::stream(
            'distract_courts.District_family.registration.district_family_handover_document',
            $data,
    ...
    {
        $case     = DistrictFamilyRegistration::with(['court', 'assignments.employee'])->findOrFail($id);
        $handover = DistrictFamilyHandover::where('family_case_id', $id)->latest()->first();
        ...
        $signatures = $handover
            ? DocumentSignature::with('signer')
                ->where('document_type', 'family_handover')
    ...
    public function store(Request $request)
    {
        ...
        $wasSubmitted = DistrictFamilyHandover::where('family_case_id', $caseId)->value('status') === 'Sug Qaatay';

        $handover = DistrictFamilyHandover::updateOrCreate(
    ...
    private function notifyHandoverApprovers(DistrictFamilyHandover $handover, $caseId): void
    {
        try {
            $case = DistrictFamilyRegistration::find($caseId);
            if (!$case) return;

            $approvers = \App\Models\User::whereHas('group.roles.permissions', function ($q) {
                $q->where('module', 'Family Case Handover Approval')->where('action', 'view');
            })->orWhereNull('group_id')->get();

            foreach ($approvers as $user) {
                $user->notify(new \App\Notifications\FamilyHandoverSubmittedNotification(
                    $handover,
                    $case->FileNo,
            ...
            \Log::error('FamilyHandoverSubmittedNotification failed: ' . $e->getMessage());
        }
    }

    public function approvalIndex(Request $request)
    {
        $baseQuery = DistrictFamilyHandover::query();
        ...
        $caseTypes = DistrictFamilyRegistration::whereNotNull('CaseType')
            ->whereIn('FCID', (clone $baseQuery)->pluck('family_case_id'))
            ->distinct()
            ->orderBy('CaseType')
            ->pluck('CaseType');
        ...
        return view('distract_courts.District_family.Conclusion.district_family_handover_approval', compact('handovers', 'pending', 'approved', 'rejected', 'draft', 'caseTypes', 'familySubCases'));
    }
     ...
     * document view page (document_type=family_handover, role=clerk).
     */
    public function approve(Request $request, $id)
    {
        ...
        $handover = DistrictFamilyHandover::where('family_case_id', $id)->latest()->firstOrFail();

        DocumentSignature::updateOrCreate(
            [
                'document_type' => 'family_handover',
            ...
            [
                ...
                'content_hash' => hash('sha256', 'family_handover|' . $handover->id . '|' . $handover->family_case_id . '|' . $newStatus),
            ]
        );

        $handover->update(['status' => $newStatus]);
        DistrictFamilyRegistration::where('FCID', $id)->update(['Status' => $newStatus]);
