...
use App\Models\DistrictFamilyHearingScripture;
...
class DistrictFamilyHearingController extends Controller
{
    public function hearingCases(Request $request)
    {
        $query = DistrictFamilyRegistration::with('handover', 'court', 'hearings')->orderByDesc('FCID');
        ...
        $hearings = DistrictFamilyHearing::all();
        $hearingStats = [
        ...
        return view('distract_courts.District_family.hearing.district_family_view_hearing_Cases', compact('records', 'statuses', 'court', 'hearingStats', 'familySubCases'));
    }

    public function hearingScripture(Request $request)
    {
        $query = DistrictFamilyRegistration::with('handover', 'court')->orderByDesc('FCID');
        ...
        return view('distract_courts.District_family.hearing.district_family_view_hearing_scripture', compact('records', 'statuses', 'hearingStats', 'familySubCases'));
    }

    public function index()
    {
        $hearings = DistrictFamilyHearing::with('familyCase.court')->orderByDesc('hearing_date')->orderByDesc('hearing_time')->get();
        $cases    = DistrictFamilyRegistration::orderByDesc('FCID')->get(['FCID', 'FileNo', 'CaseType', 'GradeCourt']);
        ...
        return view('distract_courts.District_family.hearing.district_family_view_schedule', compact('hearings', 'cases', 'courts', 'calendarEvents', 'stats'));
    }

    public function viewIndex()
    {
        $hearings = DistrictFamilyHearing::with(['familyCase.court', 'familyCase.parties', 'familyCase.assignments.employee'])->orderBy('hearing_date')->orderBy('hearing_time')->get();
        $stats = [
            'total'     => $hearings->count(),
            'scheduled' => $hearings->where('status', 'Scheduled')->count(),
            'completed' => $hearings->where('status', 'Completed')->count(),
        ...
        $familySubCases = \App\Models\CaseCategory::where('case_name', 'Qoyska')->pluck('sub_case');
        return view('distract_courts.District_family.hearing.district_family_calendar_view', compact('hearings', 'stats', 'familySubCases'));
    }
    ...
    public function create($caseId = null)
    {
        ...
        $courts       = Court::orderBy('longName')->get();
        $statuses     = StatusProcess::orderBy('name')->get();
        $selectedCase = $caseId ? DistrictFamilyRegistration::with('court')->find($caseId) : null;
        $hearing      = null;
        $caseHearings = $caseId ? DistrictFamilyHearing::where('family_case_id', $caseId)->orderBy('hearing_date')->get() : collect();
        return view('distract_courts.District_family.hearing.district_family_hearing_scheduling', compact('cases', 'courts', 'statuses', 'selectedCase', 'hearing', 'caseHearings'));
    }

    public function edit($id)
    {
        $hearing      = DistrictFamilyHearing::with('familyCase.court')->findOrFail($id);
        $cases        = DistrictFamilyRegistration::orderByDesc('FCID')->get(['FCID', 'FileNo', 'CaseType', 'GradeCourt']);
        $courts       = Court::orderBy('longName')->get();
        $statuses     = StatusProcess::orderBy('name')->get();
        $selectedCase = $hearing->familyCase;
        $caseHearings = DistrictFamilyHearing::where('family_case_id', $hearing->family_case_id)->orderBy('hearing_date')->get();
        return view('distract_courts.District_family.hearing.district_family_hearing_scheduling', compact('cases', 'courts', 'statuses', 'selectedCase', 'hearing', 'caseHearings'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $hearing = DistrictFamilyHearing::create($data);

        if ($isDraft) {
            return redirect()->route('family-hearings.edit', $hearing->id)
        ...
        DistrictFamilyRegistration::where('FCID', $request->input('family_case_id'))
            ->update(['Status' => $newStatus]);

        if ($request->expectsJson()) {
            return response()->json(['success' => true, 'message' => 'Hearing scheduled successfully.']);
        }
        return redirect()->route('family.hearing.cases')->with('success', 'Hearing scheduled successfully.');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $hearing = DistrictFamilyHearing::findOrFail($id);
        $data = $request->only('family_case_id', 'hearing_date', 'hearing_time', 'duration', 'courtroom', 'hearing_purpose');
        $data['status'] = $isDraft ? 'Draft' : $request->input('status');
        ...
        return redirect()->route('family.hearing.cases')->with('success', 'Hearing updated successfully.');
    }

    public function documentByCase($caseId)
    {
        /** @var DistrictFamilyHearing $hearing */
        $hearing = DistrictFamilyHearing::with([
            'familyCase.court',
            'familyCase.parties',
            'familyCase.lawyers.lawyer',
            'familyCase.lawyers.party',
        ...
        return view('distract_courts.District_family.hearing.district_family_hearing_document',
            array_merge(compact('hearing'), $this->hearingSignVars($hearing)));
    }

    public function createScripture($caseId)
    {
        $case      = DistrictFamilyRegistration::with('court')->findOrFail($caseId);
        $hearings  = DistrictFamilyHearing::where('family_case_id', $caseId)
                        ->whereIn('status', ['Scheduled', 'Submitted', 'Confirmed'])
                        ->orderByDesc('hearing_date')->get();
        $scripture = null;
        return view('distract_courts.District_family.hearing.district_family_add_hearing_scripture', compact('case', 'hearings', 'scripture'));
    }
    ...
    public function storeScripture(Request $request)
    {
        ...
        $request->validate([
            ...
            'hearing_id'     => $isDraft ? 'nullable' : 'required|exists:district_family_hearings,id',
            'hearing_date'   => 'nullable|date',
            'hearing_time'   => 'nullable|date_format:H:i',
        ]);

        $scripture = DistrictFamilyHearingScripture::create([
        ...
        if ($isDraft) {
            return redirect()->route('family-hearings.scripture.edit', $scripture->id)
                ->with('success', 'Qoraalka ayaa Draft ahaan lagu keydsaday.');
        }

        $newStatus = StatusProcess::where('name', 'Dhageysi')->value('name') ?? 'Dhageysi';
        DistrictFamilyRegistration::where('FCID', $scripture->family_case_id)
            ->update(['Status' => $newStatus]);

        return redirect()->route('family-hearings.scripture')
            ->with('success', 'Garmaqalka si guul leh ayaa loo gudbiiyay.');
    }

    public function editScripture($id)
    {
        $scripture = DistrictFamilyHearingScripture::findOrFail($id);
        $case      = DistrictFamilyRegistration::with('court')->findOrFail($scripture->family_case_id);
        $hearings  = DistrictFamilyHearing::where('family_case_id', $scripture->family_case_id)
                        ->whereIn('status', ['Scheduled', 'Submitted', 'Confirmed'])
                        ->orderByDesc('hearing_date')->get();
        return view('distract_courts.District_family.hearing.district_family_add_hearing_scripture', compact('case', 'hearings', 'scripture'));
    }
    ...
    {
        $scripture = DistrictFamilyHearingScripture::with([
            'familyCase.court',
            'familyCase.parties',
            'familyCase.lawyers.lawyer',
            'familyCase.lawyers.party',
        ...
        $signatures = DocumentSignature::with('signer')
            ->where('document_type', 'family_scripture')
        ...
        return view('distract_courts.District_family.hearing.district_family_document_hearing_scripture',
    ...
    public function documentPdf($id)
    {
        ...
        return \App\Support\CourtDocumentPdf::stream(
            'distract_courts.District_family.hearing.district_family_hearing_document',
            $data,
            'Hearing-' . $hearing->familyCase->FileNo . '.pdf'
        );
    }
    ...
    public function approvalStampDocument($id)
    {
        ...
        $signatures = DocumentSignature::with('signer')
            ->where('document_type', 'family_hearing_stamp')
        ...
        return view('distract_courts.Archive.district_family_approval_stamp_document',
            compact('hearing', 'case', 'clerk', 'signatures',
                    'clerkSig', 'archiveSig',
                    'myEmployee', 'myRole', 'myAlreadySigned'));
    }

    private function hearingSignVars(DistrictFamilyHearing $hearing): array
    {
        $case  = $hearing->familyCase;
        $clerk = $case->assignments->whereIn('panel_role', ['Clerk', 'Kaaliye'])->first();

        $signatures = DocumentSignature::with('signer')
            ->where('document_type', 'family_hearing')
            ->where('document_id', $hearing->id)
            ->get()
            ->keyBy('role');
        ...
        $approvalSigs   = DocumentSignature::where('document_type', 'family_hearing_stamp')
            ->where('document_id', $hearing->id)
            ->pluck('role');
    ...
    {
        DistrictFamilyHearing::findOrFail($id)->delete();
