...
use App\Models\DistrictFamilyParty;
use App\Models\DistrictFamilyRegistration;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Storage;

class DistrictFamilyPartyController extends Controller
{
    public function index(Request $request)
    {
        $caseId = $request->query('case_id');
        if (!$caseId) {
            return redirect()->route('family-registration.index')->with('error', 'Case ID is required.');
        }

        $case = DistrictFamilyRegistration::findOrFail($caseId);
        return view('distract_courts.District_family.registration.district_family_Parties_information', compact('case'));
    }

    public function getPartiesByCase($caseId)
    {
        $parties = DistrictFamilyParty::where('family_case_id', $caseId)->get();
    ...
    {
        $request->validate([
            'case_id' => 'required|exists:district_family_registrations,FCID',
        ...
        DistrictFamilyParty::where('family_case_id', $caseId)->whereNotIn('PID', $currentPIDs)->delete();
        ...
        foreach ($partyData as $index => $data) {
            ...
            if ($request->hasFile("parties.$index.passport_doc")) {
                $fields['passport_doc'] = $request->file("parties.$index.passport_doc")->store('district_family_uploads/Partts_documents', 'public');
            }
            if ($request->hasFile("parties.$index.power_of_attorney")) {
                $fields['power_of_attorney'] = $request->file("parties.$index.power_of_attorney")->store('district_family_uploads/Partts_documents', 'public');
            }

            if ($pid) {
                $fields['updatedBy']   = auth()->user()->name ?? 'Admin';
                $fields['updatedDate'] = now()->format('Y-m-d');
                DistrictFamilyParty::where('PID', $pid)->update($fields);
            } else {
                $fields['addedBy']   = auth()->user()->name ?? 'Admin';
                $fields['addedDate'] = now()->format('Y-m-d');
                DistrictFamilyParty::create($fields);
            }
        }
    ...
    public function sendNotifications($caseId)
    {
        ...
        $parties = DistrictFamilyParty::where('family_case_id', $caseId)->where('party_role', 'Plaintiff')->whereNotNull('email')->where('email', '!=', '')->get();
    ...
    {
        $party = DistrictFamilyParty::findOrFail($id);
