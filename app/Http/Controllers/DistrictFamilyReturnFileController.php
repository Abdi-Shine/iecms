...
use App\Models\DistrictFamilyReturnFile;
...
class DistrictFamilyReturnFileController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = (clone $baseQuery)->with(['court', 'returnFile', 'judgments' => fn($q) => $q->with('receipts')->orderByDesc('created_at')->limit(1)])->orderByDesc('FCID');
        ...
        return view('distract_courts.District_family.Conclusion.district_family_view_return_file',
            compact('records', 'statuses', 'stats', 'familySubCases'));
    }

    public function create($id)
    {
        $case = DistrictFamilyRegistration::with(['court', 'assignments.employee', 'documents'])->findOrFail($id);
        ...
        $existing = DistrictFamilyReturnFile::where('family_case_id', $id)->latest()->first();
        ...
        return view('distract_courts.District_family.Conclusion.district_family_add_return_file',
    ...
    public function document($id)
    {
        ...
        return view('distract_courts.District_family.Conclusion.district_family_document_return_file', $data);
    }
    ...
    public function documentReadOnly($id)
    {
        ...
        return view('distract_courts.District_family.Conclusion.district_family_document_return_file_readonly', $data);
    }
    ...
    public function documentPdf($id)
    {
        ...
        return \App\Support\CourtDocumentPdf::stream(
            'distract_courts.District_family.Conclusion.district_family_document_return_file',
            $data,
    ...
    public function store(Request $request)
    {
        ...
        DistrictFamilyReturnFile::updateOrCreate(
        ...
        if ($status === 'Qaatay') {
            ...
            DistrictFamilyRegistration::where('FCID', $caseId)->update(['Status' => $newStatus]);
        }
