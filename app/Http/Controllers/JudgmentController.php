...
use App\Models\JudgmentReceipt;
...
class JudgmentController extends Controller
{
    ...
    {
        $query = DistricCivilRegistration::with([
            'court',
        ...
        $stats = [
            ...
            'draft'     => Judgment::where('status', 'Draft')->count(),
            'submitted' => Judgment::where('status', 'Submitted')->count(),
    ...
    public function store(Request $request)
    {
        ...
        if (!$isDraft) {
            ...
            DistricCivilRegistration::where('CRID', $request->civil_case_id)
                ->update(['Status' => $newStatus]);
        }

        if ($isDraft) {
            return redirect()->route('judgments.edit', $judgment->id)
    ...
    {
        $judgment = Judgment::with('receipts')->findOrFail($id);
        $case     = DistricCivilRegistration::with('court', 'parties', 'lawyers.lawyer', 'legalRepresentatives.party', 'assignments.employee')
        ...
        $stampSigs       = JudgmentDocumentSignature::where('document_type', 'judgment_stamp')
    ...
    public function receiptsIndex(Request $request)
    {
        ...
        $stats = [
            'total'           => Judgment::count(),
            'fully_received'  => Judgment::whereHas('receipts', fn($q) => $q->groupBy('judgment_id'))->count(),
            'partial'         => 0,
            'not_received'    => Judgment::doesntHave('receipts')->count(),
        ];
    ...
    {
        $judgment = Judgment::with('civilCase.parties')->findOrFail($judgmentId);
        $party    = $judgment->civilCase->parties->where('PID', $partyId)->firstOrFail();

        $signaturePath = JudgmentReceipt::where('judgment_id', $judgmentId)
        ...
        if ($request->filled('signature_data')) {
            ...
            if ($decoded) {
                $filename = 'district_civil_uploads/judgment-signatures/' . uniqid('sig_', true) . '.png';
        ...
        $repSignaturePath = JudgmentReceipt::where('judgment_id', $judgmentId)
        ...
        if ($request->filled('rep_signature_data')) {
            ...
            $decoded = base64_decode($base64);
            if ($decoded) {
                $filename = 'district_civil_uploads/judgment-signatures/' . uniqid('rep_', true) . '.png';
                Storage::disk('public')->put($filename, $decoded);
                $repSignaturePath = $filename;
            }
        }

        JudgmentReceipt::updateOrCreate(
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$isDraft) {
            ...
            DistricCivilRegistration::where('CRID', $judgment->civil_case_id)
                ->update(['Status' => $newStatus]);
        }
