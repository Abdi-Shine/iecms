...
use App\Models\CivilCaseReturnFile;
...
class ReturnCivilFileController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = (clone $baseQuery)->with(['court', 'returnFile', 'judgments' => fn($q) => $q->with('receipts')->orderByDesc('created_at')->limit(1)])->orderByDesc('CRID');
    ...
    {
        $case = DistricCivilRegistration::with(['court', 'assignments.employee', 'documents'])->findOrFail($id);
        ...
        $existing = CivilCaseReturnFile::where('civil_case_id', $id)->latest()->first();
    ...
    private function returnFileDocumentData($id): array
    {
        ...
        $returnFile = CivilCaseReturnFile::where('civil_case_id', $id)->latest()->first();
    ...
    public function store(Request $request)
    {
        ...
        CivilCaseReturnFile::updateOrCreate(
        ...
        if ($status === 'Qaatay') {
            ...
            DistricCivilRegistration::where('CRID', $caseId)->update(['Status' => $newStatus]);
        }
