...
@section('page_title', 'Xukumista Dacwadaha — IECMS')
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Gal Ku Qorista Dacwadaha</h1>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
                <form action="{{ route('civil-case-assign.index') }}" method="GET"
                    ...
                    <div class="relative flex-1 min-w-[200px]">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Raadi lambarka kiiska ama nooca dacwadda..." class="w-full pl-10 pr-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50
                    ...
                    <select name="status" onchange="this.form.submit()" class="px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 text-neutral-600
                        ...
                        <option value="assigned" {{ request('status') == 'assigned' ? 'selected' : '' }}>Gal Ku Qoris</option>
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="font-mono text-xs font-bold px-2.5 py-0.5 rounded"
                                        style="background:rgba(82,140,190,0.1);color:#3D78AB">{{ $c->FileNo }}</span>
                                </td>

                                <td class="px-6 py-4">
                                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full"
                                        style="background:rgba(82,140,190,0.1);color:#528CBE">{{ $c->CaseType }}</span>
                                </td>

                                <td class="px-6 py-4">
                                    @if($c->SubCase)
                                        <span class="text-xs font-semibold px-2.5 py-1 rounded-full"
                                            style="background:rgba(10,40,77,0.08);color:#0A284D">{{ $c->SubCase }}</span>
                                    @else
                                        <span class="text-xs text-neutral-300">—</span>
                                    @endif
                                </td>

                                <td class="px-6 py-4 text-neutral-600 text-xs">{{ \Carbon\Carbon::parse($c->OpenDate)->format('d/m/Y') }}</td>

                                <td class="px-6 py-4 text-neutral-600 text-xs">{{ $c->NumberLetter ?: '—' }}</td>

                                <td class="px-6 py-4 text-neutral-500 text-xs">
                                    <span>{{ Str::limit($c->Remarks ?? '—', 35) }}</span>
                                </td>

                                <td class="px-6 py-4">
                                    @php
                                        $isGreen = in_array($c->Status, ['Active', 'Gal Ku Qoris', 'Qaatay']);
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('civil-case-assign.add', $c->CRID) }}" title="Xukumi Garsoore"
                                        ...
                                        <a href="{{ route('civil-registration.show', $c->CRID) }}" title="Arag Dacwadda"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                                            onmouseover="this.style.background='#528CBE';this.style.borderColor='#528CBE';this.style.color='white'"
                                            onmouseout="this.style.background='';this.style.borderColor='';this.style.color=''">
