...
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Mudeymaha Dacwadaha Madaniga </h1>
            </div>
        </div>
                ...
                <table class="w-full text-left text-sm" id="hearings-table">
                    ...
                    <tbody class="divide-y divide-neutral-100" id="table-body">
                        @forelse($hearings as $i => $h)
                            @php
                                $caseStatus = $h->civilCase->Status ?? 'N/A';
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors hearing-row"
                                ...
                                <td class="px-5 py-3.5 text-xs text-neutral-600">{{ $h->civilCase->CaseType ?? '—' }}</td>
                                <td class="px-5 py-3.5">
                                    @if($h->civilCase->SubCase ?? null)
                                        <span class="text-xs font-semibold px-2.5 py-1 rounded-full"
                                            style="background:rgba(10,40,77,0.08);color:#0A284D">{{ $h->civilCase->SubCase }}</span>
                                    @else
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    <a href="{{ route('hearings.edit', $h->id) }}" title="Edit Hearing"
