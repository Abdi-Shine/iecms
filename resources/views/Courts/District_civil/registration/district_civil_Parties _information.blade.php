...
@section('admin_main_content')
    ...
    <div class="page-wrap" x-data="partiesManager">
        ...
        <div class="page-hd">
            ...
            <a href="{{ route('civil-registration.index') }}" class="btn-back">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('partiesManager', () => ({
                ...
                async loadParties() {
                    try {
                        const res = await fetch(`/civil-case-parties/case/${this.caseId}`);
                ...
                removeParty(index) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (party.PID) {
                                try {
                                    const res = await fetch(`/civil-case-parties/${party.PID}`, {
                                        method: 'DELETE',
                ...
                async loadLegalReps() {
                    try {
                        const res = await fetch(`/civil-legal-reps/case/${this.caseId}`);
                        const data = await res.json();
                ...
                async setRep() {
                    ...
                    try {
                        ...
                        const res = await fetch('/civil-legal-reps', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: fd
                        });
                ...
                async removeRep(id) {
                    ...
                    try {
                        const res = await fetch(`/civil-legal-reps/${id}`, {
                            method: 'DELETE',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' }
                        });
                ...
                async sendNotifications() {
                    this.isSending = true;
                    try {
                        const res = await fetch(`/civil-case-parties/notify/{{ $case->CRID }}`, {
                            method: 'POST',
                            headers: {
                ...
                async saveParties() {
                    ...
                    try {
                        ...
                        const res = await fetch('/civil-case-parties', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: formData
                        });
