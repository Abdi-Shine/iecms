...
@section('page_title', 'Diiwanka Madaniga — Civil Registry')
@section('admin_main_content')
    <meta name="csrf-token" content="{{ csrf_token() }}">
    {{-- .ci, .cl, .ci-g, .action-btn and hover variants defined in resources/css/app.css --}}

    <div class="p-4 sm:p-6 w-full" x-data="civilRegManager">

        {{-- Page Header --}}
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Diiwanka Madaniga</h1>
            </div>
            <a href="{{ route('civil-registration.create') }}"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
                <form action="{{ route('civil-registration.index') }}" method="GET"
                    ...
                    <select name="sub_case" onchange="this.form.submit()"
                        ...
                        @foreach($civilSubCases as $sub)
                            <option value="{{ $sub }}" {{ request('sub_case') == $sub ? 'selected' : '' }}>
                                {{ $sub }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="px-6 py-4 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Civil Case Registry</span>
                </div>
                        ...
                        @forelse($records as $i => $r)
                            <tr class="hover:bg-neutral-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    @if($r->SubCase)
                                        <span class="text-xs font-semibold px-2.5 py-1 rounded-full"
                                            style="background:rgba(10,40,77,0.08);color:#0A284D">{{ $r->SubCase }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('civil-registration.supporting', $r->CRID) }}"
                                        ...
                                        <a href="{{ route('civil-registration.show', $r->CRID) }}" title="Case Information"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                                            onmouseover="this.style.background='#528CBE';this.style.borderColor='#528CBE';this.style.color='white'"
                                            onmouseout="this.style.background='';this.style.borderColor='';this.style.color=''">
                                            <i class="bi bi-eye text-xs"></i>
                                        </a>
                                        <a href="{{ route('civil-registration.edit', $r->CRID) }}" title="Edit"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                                            onmouseover="this.style.background='#528CBE';this.style.borderColor='#528CBE';this.style.color='white'"
                                            onmouseout="this.style.background='';this.style.borderColor='';this.style.color=''">
                                            <i class="bi bi-pencil-square text-xs"></i>
                                        </a>
                                        <button @click="deleteRecord({{ $r->CRID }})"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                            ...
                            <tr>
                                <td colspan="9" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-neutral-400 font-medium text-sm">No civil cases found.</p>
                                        <a href="{{ route('civil-registration.create') }}"
                    ...
                    deleteRecord(id) {
                        Swal.fire({ title: 'Ma hubtaa?', text: "Ma awoodi doontid inaad dib u celiso!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#DC2626', cancelButtonColor: '#528CBE', confirmButtonText: 'Haa, tirtir!', cancelButtonText: 'Jooji', customClass: { title: 'text-2xl font-bold text-neutral-800', confirmButton: 'px-6 py-2 rounded-lg font-bold text-sm mx-1', cancelButton: 'px-6 py-2 rounded-lg font-bold text-sm mx-1' }, buttonsStyling: true, showClass: { popup: 'animate__animated animate__fadeInDown animate__faster' }, hideClass: { popup: 'animate__animated animate__fadeOutUp animate__faster' } })
                            .then(async r => {
                                if (r.isConfirmed) {
                                    try {
                                        const res = await fetch(`/civil-registration/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
