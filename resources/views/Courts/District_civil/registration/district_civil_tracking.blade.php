...
@section('page_title', 'Lasocodka Dacwadaha Madaniga')
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Lasocodka Dacwadaha Madaniga</h1>
                <p class="text-sm text-neutral-500 mt-0.5">La soco dhammaan dacwadaha madaniga — dhinacyada, garsoorayaasha,
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            <a href="{{ route('civil-case-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'all'])) }}"
            ...
            <a href="{{ route('civil-case-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'Active'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
            ...
            <a href="{{ route('civil-case-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'Sug Qaatay'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
            ...
            <a href="{{ route('civil-case-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'Qaatay'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
            ...
            <a href="{{ route('civil-case-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'Closed'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
                <form action="{{ route('civil-case-tracking.index') }}" method="GET"
                    ...
                    <select name="sub_case" onchange="this.form.submit()"
                        ...
                        @forelse($records as $i => $r)
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors">
                                ...
                                <td class="px-4 py-3.5">
                                    @if($r->SubCase)
                                        <span class="text-xs font-semibold px-2 py-0.5 rounded-full"
                                            style="background:rgba(10,40,77,0.08);color:#0A284D">
                                            {{ $r->SubCase }}
                                        </span>
                                    @else
                                ...
                                <td class="px-4 py-3.5">
                                    ...
                                    @else
                                        <span class="text-xs text-neutral-300 italic">Lama qoondeynin</span>
                                    @endif
                                </td>
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-center gap-1.5">
                                        {{-- View Case --}}
                                        <a href="{{ route('civil-registration.show', $r->CRID) }}" title="Arag Faahfaahinta Dacwada"
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-24 text-center">
                                    <div class="flex flex-col items-center gap-4">
                                        ...
                                        <p class="text-neutral-400 font-semibold text-sm">Dacwad madani ah lama helin.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
