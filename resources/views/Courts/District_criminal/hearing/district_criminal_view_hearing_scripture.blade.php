...
@section('admin_main_content')
    ...
    <div class="page-wrap">
        ...
        <div class="data-card">
            ...
            {{-- Search & Filter --}}
            <form action="{{ route('criminal-hearings.scripture') }}" method="GET" class="reg-filter">
                {{-- Page size --}}
                ...
                @if(request()->anyFilled(['search', 'status']))
                    <a href="{{ route('criminal-hearings.scripture') }}" class="act-btn" title="Clear" style="width:auto;padding:0 .9rem;gap:.4rem;">
                    ...
                    <tbody>
                        @forelse($records as $i => $r)
                            ...
                            <tr>
                                ...
                                @php
                                    $latestScripture = \App\Models\DistrictCriminalHearingScripture::where('criminal_case_id', $r->CMID)->orderByDesc('created_at')->first();
                                @endphp
                                <td>
                                    <div class="td-actions">
                                        <a href="{{ route('criminal-hearings.scripture.create', $r->CMID) }}" title="Ku Dar Garmaqal"
                                            class="act-btn act-btn-poa">
                                            <i class="bi bi-journal-plus"></i>
                                        </a>
                                        @if($latestScripture && $latestScripture->status === 'Draft')
                                            <a href="{{ route('criminal-hearings.scripture.edit', $latestScripture->id) }}"
                                        ...
                                        @endif
                                        @if($latestScripture)
                                            <a href="{{ route('criminal-hearings.scripture.document', $latestScripture->id) }}"
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="empty-state">
                                        ...
                                        <p class="empty-msg">Dacwad ciqaabta ah lama helin.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
