...
<div class="p-4 sm:p-6 w-full">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('criminal-hearings.create') }}"
                ...
                <tbody class="divide-y divide-neutral-100">
                    @forelse($hearings as $i => $h)
                        ...
                        <tr class="hover:bg-neutral-50 transition-colors" id="row-{{ $h->id }}">
                            ...
                            <td class="px-5 py-3.5 text-xs text-neutral-500 max-w-[180px] truncate">{{ $h->hearing_purpose ? Str::limit(strip_tags($h->hearing_purpose), 60) : '—' }}</td>
                            <td class="px-5 py-3.5">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="flex items-center justify-center gap-1.5">
                                    <a href="{{ route('criminal-hearings.document', $h->id) }}" title="View Document"
                                    ...
                                    <button onclick="window.location='{{ route('criminal-hearings.edit', $h->id) }}'" title="Edit"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
...
$hearingsJson = $hearings->map(function($h) {
    return [
        'id'              => $h->id,
        'criminal_case_id'  => $h->criminal_case_id,
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const cal = new FullCalendar.Calendar(calEl, {
        ...
        eventClick: function(info) {
            ...
            window.location = '/criminal-hearings/' + id + '/edit';
        },
    });
    cal.render();
...
async function deleteHearing(id, btn) {
    ...
    try {
        const res  = await fetch(`/criminal-hearings/${id}`, {
