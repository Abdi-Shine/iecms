...
@section('admin_main_content')
    ...
    <div class="page-wrap" x-data="partiesManager">
        ...
        <div class="page-hd">
            ...
            <a href="{{ route('criminal-registration.index') }}" class="btn-back">
            ...
            <div style="padding:1.25rem 1.5rem;border-bottom:1px solid #f0f0f0;background:rgba(82,140,190,0.02)">
                ...
                <div class="form-row-4">
                    ...
                    <div>
                        ...
                        <div class="inp-wrap">
                            <input type="text" x-model="repEntry.rep_name" placeholder="Magaca wakiilka sharciga" class="form-inp">
                            <i class="bi bi-person inp-ico"></i>
                        </div>
                    </div>
                    <div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('partiesManager', () => ({
                ...
                async loadParties() {
                    try {
                        const res = await fetch(`/criminal-case-parties/case/${this.caseId}`);
                ...
                removeParty(index) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (party.PID) {
                                try {
                                    const res = await fetch(`/criminal-case-parties/${party.PID}`, {
                                        method: 'DELETE',
                ...
                async loadLegalReps() {
                    try {
                        const res = await fetch(`/criminal-legal-reps/case/${this.caseId}`);
                        const data = await res.json();
                ...
                async setRep() {
                    ...
                    try {
                        ...
                        fd.append('criminal_case_id',  this.caseId);
                        ...
                        const res = await fetch('/criminal-legal-reps', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: fd
                        });
                ...
                async removeRep(id) {
                    ...
                    try {
                        const res = await fetch(`/criminal-legal-reps/${id}`, {
                            method: 'DELETE',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' }
                        });
                ...
                async sendNotifications() {
                    this.isSending = true;
                    try {
                        const res = await fetch(`/criminal-case-parties/notify/{{ $case->CMID }}`, {
                            method: 'POST',
                            headers: {
                ...
                async saveParties() {
                    ...
                    try {
                        ...
                        const res = await fetch('/criminal-case-parties', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: formData
                        });
