    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('documentManager', () => ({
                ...
                async loadDocs() {
                    try {
                        const res = await fetch(`/criminal-case-documents/case/${this.caseId}`);
                ...
                removeDoc(index) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (doc.DID) {
                                try {
                                    await fetch(`/criminal-case-documents/${doc.DID}`, {
                                        method: 'DELETE',
                                        headers: {
                ...
                async saveDocs() {
                    ...
                    try {
                        ...
                        const res = await fetch('/criminal-case-documents', {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                                'Accept': 'application/json'
