...
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <a href="{{ route('family-case-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Sug Qaatay'])) }}"
            ...
            <a href="{{ route('family-case-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Qaatay'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
            ...
            <a href="{{ route('family-case-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Rejected'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
            ...
            <a href="{{ route('family-case-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Draft'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
                <form action="{{ route('family-case-handover.approval') }}" method="GET"
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors" id="row-{{ $h->id }}">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        {{-- View --}}
                                        <a href="{{ route('family-case-handover.document', $h->family_case_id) }}"
                                            title="Arag Warqadda"
                                        ...
                                        @if($hs === 'Sug Qaatay')
                                            {{-- Approve --}}
                                            <button type="button"
                                                onclick="approveSingle({{ $h->family_case_id }}, {{ $h->id }}, this)" title="Oggolow"
                                            ...
                                            {{-- Reject --}}
                                            <button type="button"
                                                onclick="rejectSingle({{ $h->family_case_id }}, {{ $h->id }}, this)" title="Diid"
                                                class="w-8 h-8 flex items-center justify-center rounded-lg border transition-all"
    ...
    <script>
        ...
        async function approveSingle(caseId, rowId, btn) {
            ...
            setLoading(btn, true);
            try {
                const res = await fetch(`/family-case-handover/${caseId}/approve`, {
                    method: 'POST',
        ...
        async function rejectSingle(caseId, rowId, btn) {
            ...
            setLoading(btn, true);
            try {
                const res = await fetch(`/family-case-handover/${caseId}/reject`, {
                    method: 'POST',
                    headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json', 'Content-Type': 'application/json' },
