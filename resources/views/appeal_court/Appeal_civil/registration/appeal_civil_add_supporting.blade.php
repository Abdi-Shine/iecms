...
@section('page_title', 'Civil Case Information')
@section('admin_main_content')

    @if(session('success'))
        <div class="mx-4 mt-4 px-4 py-3 rounded-lg text-sm font-medium" style="background:#dcfce7;color:#15803d;border:1px solid #86efac">
            <i class="bi bi-check-circle-fill me-2"></i>{{ session('success') }}
        </div>
    @endif
    @if(session('error'))
        <div class="mx-4 mt-4 px-4 py-3 rounded-lg text-sm font-medium" style="background:#fee2e2;color:#b91c1c;border:1px solid #fca5a5">
    ...
    <div class="p-4 sm:p-6 w-full" x-data="{
        deleteAssignment(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        const res = await fetch(`{{ route('appeal-civil-assign.index') }}/${id}`, {
                            method: 'DELETE',
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-neutral-500 mt-0.5">Macluumaadka Dacwada Madaniga</p>
            </div>
            <div class="flex gap-3">
                <a href="{{ route('appeal-civil-assign.index') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-neutral-600 border border-neutral-200
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden mb-6">
            ...
            <div class="px-6 py-4 border-b border-neutral-100 flex items-center gap-2">
                ...
                <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Macluumaadka Dacwada Madaniga</span>
            </div>
                ...
                <div class="px-6 py-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-4">
                        ...
                        @if($case->lower_case_no)
                            <div class="flex items-start gap-3">
                                <span class="text-xs font-semibold text-neutral-400 w-36 flex-shrink-0 pt-0.5">Lambarka
                                    Dacwadda Hore</span>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden mb-6">
            ...
            <div class="px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        @if($case->lower_court && $case->lower_case_no && $case->parties->count() === 0)
                            <form method="POST" action="{{ route('appeal-civil-registration.import-parties', $case->ACID) }}"
                                  onsubmit="return confirm('Ma hubtaa inaad soo shubto dhinacyada dacwadda hoose?')">
                                @csrf
                                <button type="submit"
                                    class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white rounded-lg transition hover:opacity-90"
                                    style="background:#16a34a">
                                    <i class="bi bi-download"></i> Soo Shub Dhinacyada Hoose
                                </button>
                            </form>
                        @endif
                        <a href="{{ route('appeal-civil-parties.index', ['case_id' => $case->ACID]) }}"
                            class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white rounded-lg transition hover:opacity-90"
                    ...
                    <tbody class="divide-y divide-neutral-100">
                        @forelse($case->parties as $index => $party)
                            <tr class="hover:bg-neutral-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    @php
                                        $roleMap = ['Plaintiff' => 'Dacwoode', 'Defendant' => 'Dacwaysane', 'Witness' => 'Markhaati', 'Other' => 'Soo Dhaxgale'];
                                ...
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <div class="flex items-center gap-2 mt-1 flex-wrap justify-center">
                                            @if($case->lower_court && $case->lower_case_no)
                                                <form method="POST" action="{{ route('appeal-civil-registration.import-parties', $case->ACID) }}"
                                                      onsubmit="return confirm('Ma hubtaa inaad soo shubto dhinacyada dacwadda hoose?')">
                                                    @csrf
                                                    <button type="submit"
                                                        class="px-4 py-2 text-xs font-semibold text-white rounded-lg hover:opacity-90 transition"
                                                        style="background:#16a34a">
                                                        <i class="bi bi-download me-1"></i> Soo Shub Dhinacyada Hoose
                                                    </button>
                                                </form>
                                            @endif
                                            <a href="{{ route('appeal-civil-parties.index', ['case_id' => $case->ACID]) }}"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden mb-6">
            ...
            <div class="px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('appeal-civil-documents.index', ['case_id' => $case->ACID]) }}"
                        class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white rounded-lg transition hover:opacity-90"
                        style="background:#528CBE">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden mb-6">
            ...
            <div class="px-6 py-4 border-b border-neutral-100 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="{{ route('appeal-civil-lawyers.index', ['case_id' => $case->ACID]) }}"
                        class="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white rounded-lg transition hover:opacity-90"
                        style="background:#528CBE">
                    ...
                    <tbody class="divide-y divide-neutral-100">
                        @forelse($case->lawyers as $index => $assign)
                            <tr class="hover:bg-neutral-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-neutral-600 text-sm">
                                    {{ $assign->assignment_date ? \Carbon\Carbon::parse($assign->assignment_date)->format('d/m/Y') : '—' }}
                                </td>
                            </tr>
                        @empty
                            <tr>
