...
@section('page_title', 'Dhaqdhaqaaqa Dacwadda — IECMS')
@section('admin_main_content')
    <meta name="csrf-token" content="{{ csrf_token() }}">


    <div class="p-4 sm:p-6 w-full" x-data="handoverManager()">

        {{-- Page Header --}}
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Dhaqdhaqaaqa Dacwadda</h1>

            </div>
            <a href="{{ route('appeal-criminal-registration.show', $case->ACMID) }}" class="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-neutral-600 border border-neutral-200
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden mb-5">
            ...
            <div class="px-6 py-5">
                ...
                <div class="mb-6">
                    <label
                        style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Dukumeentiyada
                        La Diray</label>
                    <p class="text-xs text-neutral-400 mb-3">
                        Ku qor magaca dukumeentiga iyo tiradiisa bogagga. Isticmaal badhanka si aad u ku darto ama u saarto
                        safaf.
                    </p>
                    ...
                    <div class="border border-neutral-200 rounded-xl overflow-hidden">
                        <table class="doc-table w-full bg-white">
                            <thead style="background:rgba(82,140,190,0.06)">
                                <tr>
                                    <th style="width:44%">Magaca Dukumeentiga</th>
                                    <th style="width:10%;text-align:center">Tirada</th>
                                    <th style="width:18%">Tirada Bogagga</th>
                                    <th style="width:18%">Xadka Bogagga</th>
                            ...
                            <tbody>
                                <template x-for="(doc, idx) in documents" :key="idx">
                                    <tr>
                                        <td>
                                            <div style="position:relative">
                                                <input type="text" x-model="doc.name"
                                                    placeholder="Tusaale, Heshiiska Asalka, Warbixinta Booliska..."
                                        ...
                                        <td style="text-align:center;font-weight:700;color:#0A284D;font-size:.9rem;">1</td>
                                        <td>
                                            <div style="position:relative">
                                                <input type="number" x-model="doc.pages" placeholder="0" min="0"
                                                    style="width:100%;padding:.65rem 2.25rem .65rem .875rem;font-size:.85rem;border:1.5px solid #d1d5db;border-radius:.5rem;background:#fff;color:#111827;outline:none;box-sizing:border-box;transition:border-color .15s,box-shadow .15s"
                                                    onfocus="this.style.borderColor='#528CBE';this.style.boxShadow='0 0 0 3px rgba(82,140,190,.15)'"
                                                    onblur="this.style.borderColor='#d1d5db';this.style.boxShadow='none'">
                                                <i class="bi bi-hash"
                                        ...
                                        <td>
                                            <button type="button" @click="removeDoc(idx)" class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                            ...
                            <tfoot>
                                <tr style="background:rgba(82,140,190,0.06);border-top:2px solid rgba(82,140,190,0.2)">
                                    ...
                                    <td style="font-size:.9rem;font-weight:800;color:#0A284D;padding:.55rem .875rem;text-align:center">
                                        <span x-text="documents.length"></span>
                                    </td>
                                    <td style="font-size:.78rem;font-weight:700;color:#6b7280;padding:.55rem .875rem">
                                        <span x-show="totalPages() > 0" x-text="totalPages() + ' Bog'"></span>
                                    </td>
                ...
                <div>
                    <label
                        style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Faallooyin
                        Dheeraad Ah</label>
                    <div style="position:relative">
        ...
        {{-- ═══ RETURN FILE SECTION ═══ --}}
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden mb-5" x-data="returnFileManager()">
            ...
            <div class="px-6 py-5">
                <div class="mb-6">
                    ...
                    <p class="text-xs text-neutral-400 mb-3">Ku qor magaca dukumeentiga iyo tiradiisa bogagga.</p>
                    ...
                    <div class="border border-neutral-200 rounded-xl overflow-hidden">
                        <table class="doc-table w-full bg-white">
                            ...
                            <tbody>
                                <template x-for="(doc, idx) in rfDocuments" :key="idx">
                                    <tr>
                                        <td>
                                            <div style="position:relative">
                                                <input type="text" x-model="doc.name"
                                                    placeholder="Tusaale, Xukun Asalka..."
                                                    style="width:100%;padding:.65rem 2.25rem .65rem .875rem;font-size:.85rem;border:1.5px solid #d1d5db;border-radius:.5rem;background:#fff;color:#111827;outline:none;box-sizing:border-box;transition:border-color .15s,box-shadow .15s"
                                                    onfocus="this.style.borderColor='#528CBE';this.style.boxShadow='0 0 0 3px rgba(82,140,190,.15)'"
                                                    onblur="this.style.borderColor='#d1d5db';this.style.boxShadow='none'">
                                        ...
                                        <td>
                                            <span x-text="rfPageRange(idx)"
                                                style="display:inline-block;padding:.3rem .75rem;font-size:.8rem;font-weight:700;font-family:monospace;background:rgba(82,140,190,0.08);color:#528CBE;border-radius:.5rem;border:1px solid rgba(82,140,190,0.2)">
                                            </span>
                                        </td>
                                        <td>
                                            <button type="button" @click="rfRemoveDoc(idx)"
                            ...
                            <tfoot>
                                <tr style="background:rgba(82,140,190,0.06);border-top:2px solid rgba(82,140,190,0.2)">
                                    ...
                                    <td style="font-size:.9rem;font-weight:800;color:#0A284D;padding:.55rem .875rem;text-align:center">
                                        <span x-text="rfDocuments.length"></span>
                                    </td>
                                    <td style="font-size:.78rem;font-weight:700;color:#6b7280;padding:.55rem .875rem">
                                        <span x-show="rfTotalPages() > 0" x-text="rfTotalPages() + ' Bog'"></span>
                                    </td>
                                    <td style="padding:.55rem .875rem;font-size:.78rem;font-weight:700;color:#6b7280">
                                        <span x-show="rfTotalPages() > 0" x-text="'Bog 1 — ' + rfTotalPages()"></span>
                                    </td>
                                    <td></td>
                                </tr>
                            </tfoot>
                    ...
                    <button type="button" @click="rfAddDoc()"
                        class="mt-3 flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-xl border transition-all"
                        style="color:#528CBE;border-color:#528CBE;background:transparent"
                        onmouseover="this.style.background='#528CBE';this.style.color='white'"
                        onmouseout="this.style.background='transparent';this.style.color='#528CBE'">
                ...
                <div>
                    ...
                    <textarea x-model="rfNotes" placeholder="Faallooyin ama xusus kale..."
                        style="width:100%;padding:.65rem .875rem;font-size:.85rem;border:1.5px solid #d1d5db;border-radius:.625rem;background:#fff;color:#111827;outline:none;box-sizing:border-box;resize:vertical;min-height:90px;transition:border-color .15s,box-shadow .15s"
                        onfocus="this.style.borderColor='#528CBE';this.style.boxShadow='0 0 0 3px rgba(82,140,190,.15)'"
                        onblur="this.style.borderColor='#d1d5db';this.style.boxShadow='none'"></textarea>
                </div>

                {{-- Return File Save Buttons --}}
                <div style="display:flex;justify-content:flex-end;gap:.5rem;margin-top:1.25rem">
                    <button type="button" @click="rfSave('Draft')" :disabled="rfSaving"
                        style="display:flex;align-items:center;gap:.5rem;padding:.6rem 1.5rem;font-size:.85rem;font-weight:700;color:#528CBE;background:white;border:1.5px solid #528CBE;border-radius:.625rem;cursor:pointer;transition:opacity .15s"
                        :class="{'opacity-70 cursor-not-allowed': rfSaving}">
                        <i class="bi bi-floppy" x-show="!rfSaving"></i>
                        <span x-text="rfSaving ? 'Waa La Kaydiyaa...' : 'Keydi Muswaaada'"></span>
                    </button>
                    <button type="button" @click="rfSave('Sug Qaatay')" :disabled="rfSaving"
                        ...
                        :class="{'opacity-70 cursor-not-allowed': rfSaving}">
                        <i class="bi bi-send" x-show="!rfSaving"></i>
                        <span x-text="rfSaving ? 'Waa La Diraa...' : 'Dir Xaqiijinta Kaaliyaha'"></span>
                    </button>
                </div>
            </div>
        </div>
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.5rem;padding-bottom:1.5rem">
            <a href="{{ route('appeal-criminal-registration.show', $case->ACMID) }}"
            ...
            <div class="flex items-center gap-2">
                <button type="button" @click="save('Draft')" :disabled="saving"
                    ...
                    <span x-text="saving ? 'Waa La Kaydiyaa...' : 'Keydi Muswaaada'"></span>
                </button>
                <button type="button" @click="save('Sug Qaatay')" :disabled="saving"
                    style="display:flex;align-items:center;gap:.5rem;padding:.6rem 1.75rem;font-size:.85rem;font-weight:700;
                                                           ...
                                                           box-shadow:0 4px 14px rgba(82,140,190,.4);transition:opacity .15s" :class="{'opacity-70 cursor-not-allowed': saving}"
                    onmouseover="if(!this.disabled)this.style.opacity='.88'" onmouseout="this.style.opacity='1'">
                    <i class="bi bi-send" x-show="!saving"></i>
                    <i class="bi bi-arrow-repeat" x-show="saving" style="display:none"></i>
                    <span x-text="saving ? 'Waa La Diraa...' : 'Dir Xaqiijinta Kaaliyaha'"></span>
                </button>
            </div>
        </div>
    ...
    <script>
        function handoverManager() {
            ...
            return {
                ...
                documents: (existing && existing.documents && existing.documents.length)
                    ? existing.documents
                    : (@json($caseDocuments->count() > 0 ? $caseDocuments : collect([['name'=>'','pages'=>0]]))),
                specialInstructions: existing?.special_instructions ?? '',
                additionalNotes: existing?.additional_notes ?? '',

                addDoc() {
                    this.documents.push({ name: '', pages: '' });
                },
                ...
                async save(status = 'Draft') {
                    this.saving = true;
                    try {
                        const res = await fetch('{{ route('appeal-criminal-handover.store') }}', {
                            method: 'POST',
                            headers: {
        ...
        function returnFileManager() {
            const rfExisting = @json($existingReturnFile);
            const rfDocs     = @json($returnFileDocuments);

            return {
                rfSaving: false,
                caseId: {{ $case->ACMID }},
                rfDocuments: (rfExisting && rfExisting.documents && rfExisting.documents.length)
                    ? rfExisting.documents
                    : (rfDocs.length ? rfDocs : [{ name: '', pages: 0 }]),
                rfNotes: rfExisting?.additional_notes ?? '',

                rfAddDoc() { this.rfDocuments.push({ name: '', pages: 0 }); },
                rfRemoveDoc(idx) { if (this.rfDocuments.length > 1) this.rfDocuments.splice(idx, 1); },

                rfPageCount(p) {
                    const v = String(p ?? '').trim();
                    if (v.includes('-')) { const [s, e] = v.split('-').map(Number); return Math.max(0, (e||0)-(s||0)+1); }
                    return Math.max(0, parseInt(v) || 0);
                },
                rfPageRange(idx) {
                    let running = 0;
                    for (let i = 0; i < idx; i++) running += this.rfPageCount(this.rfDocuments[i].pages);
                    const count = this.rfPageCount(this.rfDocuments[idx].pages);
                    if (!count) return '—';
                    const start = running + 1, end = running + count;
                    return start === end ? String(start) : start + '-' + end;
                },
                rfTotalPages() { return this.rfDocuments.reduce((s, d) => s + this.rfPageCount(d.pages), 0); },

                async rfSave(status = 'Draft') {
                    this.rfSaving = true;
                    try {
                        const res = await fetch('{{ route('appeal-criminal-return-file.store') }}', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: JSON.stringify({ criminal_case_id: this.caseId, documents: this.rfDocuments, additional_notes: this.rfNotes, status: status }),
                        });
                        const data = await res.json();
                        if (res.ok && data.success) {
                            Swal.fire({ title: 'La Keydsaday!', text: data.message, icon: 'success', confirmButtonText: 'Hagaag', confirmButtonColor: '#528CBE' });
                        } else { throw new Error(data.message || 'Save failed.'); }
                    } catch (e) {
                        Swal.fire({ title: 'Khalad!', text: e.message, icon: 'error', confirmButtonColor: '#DC2626' });
                    } finally { this.rfSaving = false; }
                },
            };
        }
    </script>
