...
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-neutral-500 mt-0.5">Dib u eeg, oggolow ama diibi codsiyada dhaqdhaqaaqa dacwadaha ciqaabta</p>
            </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <a href="{{ route('appeal-criminal-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Sug Qaatay'])) }}"
            ...
            <a href="{{ route('appeal-criminal-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Qaatay'])) }}"
            ...
            <a href="{{ route('appeal-criminal-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Rejected'])) }}"
            ...
            <a href="{{ route('appeal-criminal-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Draft'])) }}"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
                <form action="{{ route('appeal-criminal-handover.approval') }}" method="GET"
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        {{-- View --}}
                                        <a href="{{ route('appeal-criminal-handover.document', $h->criminal_case_id) }}"
                                           title="Arag Warqadda"
                                        ...
                                        {{-- Approve --}}
                                        <button type="button"
                                                onclick="approveSingle({{ $h->criminal_case_id }}, {{ $h->id }}, this)"
                                                title="Oggolow"
                                        ...
                                        {{-- Reject --}}
                                        <button type="button"
                                                onclick="rejectSingle({{ $h->criminal_case_id }}, {{ $h->id }}, this)"
                                                title="Diid"
                                                class="w-8 h-8 flex items-center justify-center rounded-lg border transition-all"
    ...
    <script>
        ...
        async function approveSingle(caseId, rowId, btn) {
            ...
            setLoading(btn, true);
            try {
                const res  = await fetch(`/appeal-criminal-handover/${caseId}/approve`, {
                    method: 'POST',
        ...
        async function rejectSingle(caseId, rowId, btn) {
            ...
            setLoading(btn, true);
            try {
                const res  = await fetch(`/appeal-criminal-handover/${caseId}/reject`, {
                    method: 'POST',
                    headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json', 'Content-Type': 'application/json' },
