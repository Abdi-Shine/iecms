...
@section('admin_main_content')
    ...
    <div class="page-wrap" x-data="documentManager">
        ...
        <datalist id="criminal-doc-types">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('documentManager', () => ({
                ...
                async loadDocs() {
                    try {
                        const res = await fetch(`/appeal-criminal-documents/case/${this.caseId}`);
                ...
                removeDoc(index) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (doc.DID) {
                                try {
                                    await fetch(`/appeal-criminal-documents/${doc.DID}`, {
                                        method: 'DELETE',
                                        headers: {
                ...
                async saveDocs() {
                    ...
                    try {
                        ...
                        const res = await fetch('/appeal-criminal-documents', {
                            method: 'POST',
                            headers: {
                                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                                'Accept': 'application/json'
