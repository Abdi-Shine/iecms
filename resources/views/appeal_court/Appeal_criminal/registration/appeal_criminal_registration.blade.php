...
@section('admin_main_content')
    ...
    <div class="page-wrap" x-data="criminalRegManager">
        ...
        <div class="page-hd">
            <div class="page-hd-left">
                ...
                <div>
                    ...
                    <p class="page-sub">Maxkamadda Rafcaanka — Diiwaangelinta dacwadaha ciqaabta</p>
                </div>
            </div>
        ...
        <div class="data-card">
            ...
            <div class="reg-filter">
                <form action="{{ route('appeal-criminal-registration.index') }}" method="GET"
                    ...
                    @if(request()->anyFilled(['search', 'status', 'case_type']))
                        <a href="{{ route('appeal-criminal-registration.index') }}" class="btn-cancel">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="td-actions">
                                        <a href="{{ route('appeal-criminal-registration.supporting', $r->ACMID) }}"
                                ...
                                <div class="inp-wrap" style="position:relative">
                                    <template x-if="rafcaanCasesList.length > 0">
                                        <select x-model="f.lower_case_no" class="form-sel">
                                            ...
                                            <template x-for="c in rafcaanCasesList" :key="c.CMID">
                        ...
                        <div style="margin-bottom:1.1rem">
                            ...
                            <div class="inp-wrap">
                                <select x-model="f.SubCase" class="form-sel">
                                    ...
                                    @foreach($criminalSubCases as $sub)
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('criminalRegManager', () => ({
                ...
                f: {
                    ...
                    CaseType: 'Ciqaabta', SubCase: '', OpenDate: '{{ date("Y-m-d") }}',
                    NumberLetter: '', LegalBasis: 'Dacwadda Ciqaabta', Orders_Requested: '',
                ...
                async fetchRafcaanCases(courtcode) {
                    ...
                    try {
                        const res  = await fetch(`/appeal-criminal-registration/rafcaan-cases/${encodeURIComponent(courtcode)}`, { headers: { 'Accept': 'application/json' } });
                ...
                openModal(id = null, RegisterNo = '', FileNo = '', GradeCourt = '{{ $userCourtID ?? "" }}',
                          CaseType = 'Ciqaabta', SubCase = '', OpenDate = '{{ date("Y-m-d") }}',
                          NumberLetter = '', LegalBasis = 'Dacwadda Ciqaabta',
                ...
                closeModal() {
                    ...
                    this.f = { RegisterNo: '', FileNo: '', GradeCourt: '{{ $userCourtID ?? "" }}', CaseType: 'Ciqaabta', SubCase: '', OpenDate: '{{ date("Y-m-d") }}', NumberLetter: '', LegalBasis: 'Dacwadda Ciqaabta', Orders_Requested: '', Remarks: '', Status: 'Diwaan Galin', lower_court: '', lower_case_no: '' };
                },
                async generateFileNo(courtcode) {
                    if (!courtcode || this.isEditMode) return;
                    this.f.FileNo = '';
                    try {
                        const res = await fetch(`/appeal-criminal-registration/next-fileno/${encodeURIComponent(courtcode)}`, { headers: { 'Accept': 'application/json' } });
                        if (!res.ok) return;
                        const data = await res.json();
                ...
                async submitForm() {
                    ...
                    try {
                        const url    = this.isEditMode ? `/appeal-criminal-registration/${this.currentId}` : '/appeal-criminal-registration';
                ...
                deleteRecord(id) {
                    Swal.fire({
                        ...
                        if (r.isConfirmed) {
                            try {
                                const res  = await fetch(`/appeal-criminal-registration/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                                const data = await res.json();
                                if (res.ok && data.success) {
