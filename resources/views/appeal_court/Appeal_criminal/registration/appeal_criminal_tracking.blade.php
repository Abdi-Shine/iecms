...
@section('page_title', 'Criminal Case Tracking')
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Criminal Case Tracking</h1>
                <p class="text-sm text-neutral-500 mt-0.5">Monitor all criminal cases — parties, judges, handovers, and status at a glance</p>
            </div>
        </div>

        {{-- KPI Cards --}}
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-6">
            <a href="{{ route('appeal-criminal-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'all'])) }}"
               class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer" title="All cases">
            ...
            <a href="{{ route('appeal-criminal-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'Active'])) }}"
               class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer" title="Filter active">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <a href="{{ route('appeal-criminal-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'Sug Qaatay'])) }}"
               class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer" title="Filter pending handover">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <a href="{{ route('appeal-criminal-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'Qaatay'])) }}"
               class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer" title="Filter completed">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <a href="{{ route('appeal-criminal-tracking.index', array_merge(request()->except(['status', 'page']), ['status' => 'Closed'])) }}"
               class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer" title="Filter closed">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                     style="background:rgba(239,68,68,0.1)">
                    <i class="bi bi-archive text-xl" style="color:#DC2626"></i>
                </div>
                <div>
                    <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Closed</p>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
                <form action="{{ route('appeal-criminal-tracking.index') }}" method="GET"
                    ...
                    <div class="relative flex-1 min-w-[200px]">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                               placeholder="Search by file no, party name, telephone, or mother name..."
            ...
            <div class="px-6 py-4 flex items-center justify-between border-b border-neutral-100">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Case Tracking Records</span>
                </div>
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr style="background:rgba(82,140,190,0.06)">
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500 w-12">No#</th>
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">File No</th>
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Case Type</th>
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Open Date</th>
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Judge</th>
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Status</th>
                            <th class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500 text-center w-28">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-neutral-100">
                        @forelse($records as $i => $r)
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors">
                                ...
                                <td class="px-4 py-3.5">
                                    <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wide whitespace-nowrap"
                                          style="background:{{ $cStatusBg }};color:{{ $cStatusColor }}">
                                        <span class="w-1.5 h-1.5 rounded-full inline-block" style="background:{{ $cStatusColor }}"></span>
                                        {{ $cs }}
                                    </span>
                                </td>
                                ...
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-center gap-1.5">
                                        {{-- View Case --}}
                                        <a href="{{ route('appeal-criminal-registration.show', $r->ACMID) }}"
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-24 text-center">
                                    <div class="flex flex-col items-center gap-4">
                                        ...
                                        <p class="text-neutral-400 font-semibold text-sm">No criminal cases found.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
