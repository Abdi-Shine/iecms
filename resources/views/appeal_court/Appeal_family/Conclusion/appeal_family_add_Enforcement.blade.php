...
@section('page_title', $enforcement ? 'Wax ka Badal Dhaqan Galka' : 'Dhaqan Galka Dacwadaha')
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1.5rem">
            <div>
                <h1 style="font-size:1.25rem;font-weight:700;color:#111827;margin:0 0 .25rem">
                    {{ $enforcement ? 'Wax ka Badal Dhaqan Galka' : 'Dhaqan Galka Dacwadaha' }}
                </h1>
                <p style="font-size:.8125rem;color:#6b7280;margin:0">
                    Dacwada: <strong style="color:#374151">{{ $case->FileNo }}</strong>
                </p>
                @if($enforcement && $enforcement->status === 'Draft')
            ...
            <a href="{{ route('appeal-family-enforcement.index') }}" class="btn-back">
                <i class="bi bi-arrow-left"></i> Dib ugu Noqo
            </a>
        </div>

        <form method="POST" action="{{ route('appeal-family-enforcement.store') }}" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="family_case_id" value="{{ $case->AFCID }}">
            ...
            {{-- Section 2: Enforcement Details --}}
            <div class="sec-card">
                <div class="sec-hd">
                    <i class="bi bi-hammer" style="color:#528CBE;font-size:.875rem"></i>
                    <span class="sec-title">Faahfaahinta Dhaqan Galka</span>
                </div>
                <div class="sec-bd">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                        <div>
                            <label class="form-lbl">Xaladada Dacwada <span class="req">*</span></label>
                            <div class="inp-wrap">
                                <select name="enforcement_type" class="form-sel">
                                    <option value="">— Dooro —</option>
                                    @foreach($caseStatus as $s)
                                        <option value="{{ $s->name }}"
                                            {{ old('enforcement_type', $enforcement?->enforcement_type ?? $defaultStatus) === $s->name ? 'selected' : '' }}>
                                            {{ $s->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            <label class="form-lbl">Taariikhda Dhaqan Galka <span class="req">*</span></label>
                            <div class="inp-wrap">
                                <input type="date" name="enforcement_date"
                                    value="{{ old('enforcement_date', $enforcement?->enforcement_date ? \Carbon\Carbon::parse($enforcement->enforcement_date)->format('Y-m-d') : now()->format('Y-m-d')) }}"
                                    class="form-inp">
                    ...
                    <div>
                        ...
                        <textarea name="additional_notes" class="form-textarea" rows="3"
                            placeholder="Faallo kale...">{{ old('additional_notes', $enforcement->additional_notes ?? '') }}</textarea>
                    </div>
                    ...
                    <div>
                        <label class="form-lbl">
                            ...
                            Dukuumintiga Dhaqan Galka <span class="sec-opt">(ikhtiyaari — PDF, Word, JPG)</span>
                        </label>
                        ...
                        @if($enforcement?->attachment)
                            <div style="display:flex;align-items:center;gap:10px;padding:.625rem .875rem;background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;margin-bottom:.5rem">
                                ...
                                <a href="{{ asset('storage/' . $enforcement->attachment) }}" target="_blank" download
                        ...
                        <label style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.5rem;
                            ...
                            <span style="font-size:.875rem;font-weight:600;color:#374151">
                                {{ $enforcement?->attachment ? 'Bedel Faylka' : 'Dooro Faylka' }}
                            </span>
            ...
            <div class="form-actions">
                <a href="{{ route('appeal-family-enforcement.index') }}" class="btn-cancel">
                ...
                <div style="display:flex;gap:.75rem">
                    <button type="submit" name="status" value="Draft" class="btn-save"
                        ...
                        {{ $enforcement ? 'Cusboonaysii Muswaadda' : 'Keydi Muswaadda' }}
                    </button>
                    <button type="submit" name="status" value="Submitted" class="btn-submit">
                        <i class="bi bi-send"></i>
                        {{ $enforcement ? 'Cusboonaysii & Gudbi' : 'Gudbi Dhaqan Galka' }}
                    </button>
                </div>
            </div>
