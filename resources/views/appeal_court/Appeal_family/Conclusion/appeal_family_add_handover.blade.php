...
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full" x-data="handoverManager()">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="{{ route('appeal-family-registration.show', $case->AFCID) }}" class="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-neutral-600 border border-neutral-200
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.5rem;padding-bottom:1.5rem">
            <a href="{{ route('appeal-family-registration.show', $case->AFCID) }}"
        ...
        function handoverManager() {
            ...
            return {
                ...
                async save(status = 'Draft') {
                    this.saving = true;
                    try {
                        const res = await fetch('{{ route('appeal-family-handover.store') }}', {
                            method: 'POST',
                            headers: {
        ...
        function returnFileManager() {
            ...
            return {
                ...
                async rfSave(status = 'Draft') {
                    this.rfSaving = true;
                    try {
                        const res = await fetch('{{ route('appeal-family-return-file.store') }}', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: JSON.stringify({ family_case_id: this.caseId, documents: this.rfDocuments, additional_notes: this.rfNotes, status: status }),
                        });
                        const data = await res.json();
                        if (res.ok && data.success) {
