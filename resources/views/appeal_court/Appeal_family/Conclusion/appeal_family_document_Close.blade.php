...
<div class="no-print doc-toolbar">
    ...
    <div class="doc-toolbar-actions">
        ...
        {{-- Stamp Request — the modal (openStampModal/submitStamp below) posts to the
             generic document.sign route; there's no separate "stamp-request" route,
             so both the eligible-clerk and fallback cases share the same modal. --}}
        @if($isStampApproved)
            <span style="display:inline-flex;align-items:center;gap:.4rem;padding:.45rem 1.1rem;font-size:.82rem;font-weight:700;color:#065f46;background:rgba(16,185,129,.12);border-radius:.5rem;border:1.5px solid rgba(16,185,129,.3);">
                <i class="bi bi-patch-check-fill"></i> Shaabadda La Ansixiyay
            </span>
        @elseif($isStampRequested)
            <span style="display:inline-flex;align-items:center;gap:.4rem;padding:.45rem 1.1rem;font-size:.82rem;font-weight:700;color:#92600a;background:rgba(240,180,60,.12);border-radius:.5rem;border:1.5px solid rgba(240,180,60,.35);">
                <i class="bi bi-hourglass-split"></i> Codsi Sugaya Ansixinta
            </span>
        @else
            <button onclick="openStampModal()"
                style="display:inline-flex;align-items:center;gap:.4rem;padding:.45rem 1.1rem;font-size:.82rem;font-weight:700;color:#0A284D;background:rgba(10,40,77,.07);border:1.5px solid rgba(10,40,77,.2);border-radius:.5rem;cursor:pointer">
                <i class="bi bi-archive"></i> Codso Shaabadda
            </button>
        @endif
...
<div id="judgment-doc" class="doc-page-multi">
    ...
    <div class="doc-body">
        ...
        <p style="text-align:center;font-weight:700;font-size:13px;margin-bottom:12px;letter-spacing:.04em">QAYBTA QOYSKA</p>
        ...
        <p class="doc-title">QARAARKA DACWADA QOYSKA</p>
        ...
        <p class="doc-para doc-para-mt">
            Waxay u fadhiisatay dhageysiga dacwad qoyska ah ee summadeedu tahay:
            <strong>{{ $ref }},</strong>
...
function submitSign() {
    ...
    fetch('{{ route("document.sign", ["type" => "close_case", "id" => $judgment->id]) }}', {
        method: 'POST',
...
document.addEventListener('keydown', function(e) { if (e.key === 'Escape') { closeSignModal(); closeStampModal(); } });

function openStampModal() {
    var m = document.getElementById('stamp-modal');
    if (m) { m.style.display = 'flex'; document.getElementById('stamp-password').focus(); }
}
function closeStampModal() {
    var m = document.getElementById('stamp-modal');
    if (m) {
        m.style.display = 'none';
        document.getElementById('stamp-password').value = '';
        document.getElementById('stamp-error').style.display = 'none';
    }
}
function submitStamp() {
    var btn  = document.getElementById('stamp-confirm-btn');
    var pass = document.getElementById('stamp-password').value;
    var err  = document.getElementById('stamp-error');
    if (!pass) { err.textContent = 'Fadlan erayga sirta ah geli.'; err.style.display = 'block'; return; }
    btn.disabled = true;
    btn.innerHTML = '<i class="bi bi-hourglass-split"></i> Sugaya...';
    fetch('{{ route("document.sign", ["type" => "close_case_stamp", "id" => $closeCase->id]) }}', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') },
        body: JSON.stringify({ password: pass, role: 'kaaliye' })
    })
    .then(function(r) { if (!r.ok && r.status !== 422) throw new Error('server'); return r.json(); })
    .then(function(data) {
        if (data.success) {
            ...
            var toast = document.createElement('div');
            toast.style.cssText = 'position:fixed;top:24px;right:24px;z-index:99999;background:#0A284D;color:white;padding:14px 22px;border-radius:12px;font-weight:700;font-size:.875rem;box-shadow:0 8px 30px rgba(0,0,0,.2);display:flex;align-items:center;gap:.6rem';
            toast.innerHTML = '<i class="bi bi-patch-check-fill" style="font-size:1.1rem;color:#10b981"></i> Codsiga Summadda si guul leh ayaa loo diray!';
            document.body.appendChild(toast);
            setTimeout(function() { window.location.reload(); }, 1400);
        } else {
            err.textContent = data.message || 'Khalad ayaa dhacay.';
            err.style.display = 'block';
            btn.disabled = false;
            btn.innerHTML = '<i class="bi bi-archive"></i> Xaqiiji Codsiga';
        }
    })
    .catch(function() {
        err.textContent = 'Xiriirka shabakadda waa xumaaday. Isku day mar kale.';
...
{{-- ═══ STAMP REQUEST MODAL ═══ --}}
@if($myStampRole === 'kaaliye')
<div id="stamp-modal"
     style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.45);align-items:center;justify-content:center">
    <div style="background:white;border-radius:16px;padding:2rem;width:100%;max-width:420px;box-shadow:0 20px 60px rgba(0,0,0,.2);margin:1rem">

        <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:1.25rem">
            <div style="width:40px;height:40px;background:#528CBE;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0">
                <i class="bi bi-archive" style="color:white;font-size:1.1rem"></i>
            </div>
            <div>
                <h3 style="font-size:1rem;font-weight:800;color:#111827;margin:0">Codso Summadda Archiifka</h3>
                <p style="font-size:.78rem;color:#6b7280;margin:0">{{ $ref }} · {{ $judgment->judgment_type ?? 'Qaraarka' }}</p>
            </div>
            <button onclick="closeStampModal()"
                    style="margin-left:auto;background:none;border:none;font-size:1.2rem;color:#9ca3af;cursor:pointer">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>
        ...
        <div style="margin-bottom:1.25rem">
            ...
            <input type="password" id="stamp-password" placeholder="Geli eraygaaga sirta ah..."
                   style="width:100%;padding:.65rem .875rem;font-size:.875rem;border:1.5px solid #d1d5db;border-radius:8px;background:white;color:#111827;outline:none;box-sizing:border-box"
                   onfocus="this.style.borderColor='#528CBE'" onblur="this.style.borderColor='#d1d5db'">
        </div>
        ...
        <div style="display:flex;gap:.75rem">
            ...
            <button id="stamp-confirm-btn" onclick="submitStamp()"
                    style="flex:2;padding:.7rem;font-size:.82rem;font-weight:700;color:white;background:#528CBE;border:none;border-radius:8px;cursor:pointer">
                <i class="bi bi-archive"></i> Xaqiiji Codsiga
            </button>
        </div>
    </div>
</div>
