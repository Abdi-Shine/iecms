...
@section('admin_main_content')
    ...
    <div class="page-wrap">
        ...
        <div class="page-hd">
            <div class="page-hd-left">
                ...
                <div>
                    ...
                    <p class="page-sub">Maamul iyo fulinta dhageysiyada dacwadaha qoyska</p>
                </div>
            </div>
        </div>
        ...
        <div class="data-card">
            ...
            {{-- Search & Filter --}}
            <form action="{{ route('appeal-family-hearings.scripture') }}" method="GET" class="reg-filter">
                {{-- Page size --}}
                ...
                @if(request()->anyFilled(['search', 'status']))
                    <a href="{{ route('appeal-family-hearings.scripture') }}" class="act-btn" title="Clear" style="width:auto;padding:0 .9rem;gap:.4rem;">
                    ...
                    <tbody>
                        @forelse($records as $i => $r)
                            ...
                            <tr>
                                ...
                                @php
                                    $latestScripture = \App\Models\HearingScripture::where('family_case_id', $r->AFCID)->orderByDesc('created_at')->first();
                                @endphp
                                <td>
                                    <div class="td-actions">
                                        <a href="{{ route('appeal-family-hearings.scripture.create', $r->AFCID) }}" title="Ku Dar Garmaqal"
                                            class="act-btn act-btn-poa">
                                            <i class="bi bi-journal-plus"></i>
                                        </a>
                                        @if($latestScripture && $latestScripture->status === 'Draft')
                                            <a href="{{ route('appeal-family-hearings.scripture.edit', $latestScripture->id) }}"
                                        ...
                                        @endif
                                        @if($latestScripture)
                                            <a href="{{ route('appeal-family-hearings.scripture.document', $latestScripture->id) }}"
