...
@section('admin_main_content')
    ...
    <div class="page-wrap" x-data="familyRegManager">
        ...
        <div class="page-hd">
            <div class="page-hd-left">
                ...
                <div>
                    ...
                    <p class="page-sub">Maxkamadda Rafcaanka — Diiwaangelinta dacwadaha qoyska</p>
                </div>
            </div>
        ...
        <div class="data-card">
            ...
            <div class="reg-filter">
                <form action="{{ route('appeal-family-registration.index') }}" method="GET"
                    ...
                    @if(request()->anyFilled(['search', 'status', 'case_type']))
                        <a href="{{ route('appeal-family-registration.index') }}" class="btn-cancel">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="td-actions">
                                        <a href="{{ route('appeal-family-registration.supporting', $r->AFCID) }}"
                                ...
                                <div class="inp-wrap" style="position:relative">
                                    <template x-if="rafcaanCasesList.length > 0">
                                        <select x-model="f.lower_case_no" class="form-sel">
                                            ...
                                            <template x-for="c in rafcaanCasesList" :key="c.FCID">
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('familyRegManager', () => ({
                ...
                f: {
                    ...
                    CaseType: 'Qoyska', SubCase: '', OpenDate: '{{ date("Y-m-d") }}',
                    NumberLetter: '', LegalBasis: 'Dacwadda Qoyska', Orders_Requested: '',
                ...
                async fetchRafcaanCases(courtcode) {
                    ...
                    try {
                        const res  = await fetch(`/appeal-family-registration/rafcaan-cases/${encodeURIComponent(courtcode)}`, { headers: { 'Accept': 'application/json' } });
                ...
                openModal(id = null, RegisterNo = '', FileNo = '', GradeCourt = '{{ $userCourtID ?? "" }}',
                          CaseType = 'Qoyska', SubCase = '', OpenDate = '{{ date("Y-m-d") }}',
                          NumberLetter = '', LegalBasis = 'Dacwadda Qoyska',
                ...
                closeModal() {
                    ...
                    this.f = { RegisterNo: '', FileNo: '', GradeCourt: '{{ $userCourtID ?? "" }}', CaseType: 'Qoyska', SubCase: '', OpenDate: '{{ date("Y-m-d") }}', NumberLetter: '', LegalBasis: 'Dacwadda Qoyska', Orders_Requested: '', Remarks: '', Status: 'Diwaan Galin', lower_court: '', lower_case_no: '' };
                },
                async generateFileNo(courtcode) {
                    if (!courtcode || this.isEditMode) return;
                    this.f.FileNo = '';
                    try {
                        const res = await fetch(`/appeal-family-registration/next-fileno/${encodeURIComponent(courtcode)}`, { headers: { 'Accept': 'application/json' } });
                        if (!res.ok) return;
                        const data = await res.json();
                ...
                async submitForm() {
                    ...
                    try {
                        const url    = this.isEditMode ? `/appeal-family-registration/${this.currentId}` : '/appeal-family-registration';
                ...
                deleteRecord(id) {
                    Swal.fire({
                        ...
                        if (r.isConfirmed) {
                            try {
                                const res  = await fetch(`/appeal-family-registration/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                                const data = await res.json();
                                if (res.ok && data.success) {
