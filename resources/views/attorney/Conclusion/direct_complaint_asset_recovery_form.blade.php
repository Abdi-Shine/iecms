...
@section('page_title', 'Soo Celinta Hantida — ' . $case->case_number)
@section('admin_main_content')

@php
    $d = $case->assetRecovery;
    $isComplete = (bool) $d;
...
<div class="p-4 sm:p-6 w-full">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-neutral-100 flex items-center gap-2"
            ...
            <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Soo Celinta Hantida</span>
        </div>

        <form action="{{ route('attorney-cases.workflow.arrest-decision.asset-recovery.store', $case->ACID) }}" method="POST"
            ...
            <div class="rounded-2xl border border-neutral-200 p-5 mb-5">
                <h3 class="flex items-center gap-2 text-sm font-bold text-neutral-800 mb-4">
                    <i class="bi bi-safe2 text-primary-400"></i> Faahfaahinta Hantida
                </h3>
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Sharaxaadda Hantida</label>
                <textarea name="asset_description" rows="3"
                    class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 focus:outline-none focus:border-primary-400 transition-all mb-4">{{ old('asset_description', $d->asset_description ?? '') }}</textarea>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Qiimaha La Qiyaasay</label>
                        <input type="text" name="estimated_value" placeholder="tusaale: $5,000"
                            value="{{ old('estimated_value', $d->estimated_value ?? '') }}"
                    ...
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Goobta Hantida</label>
                        <input type="text" name="asset_location"
                            value="{{ old('asset_location', $d->asset_location ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                </div>
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Faahfaahinta Milkiilaha</label>
                <textarea name="ownership_details" rows="2"
                    class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 focus:outline-none focus:border-primary-400 transition-all">{{ old('ownership_details', $d->ownership_details ?? '') }}</textarea>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 p-5 mb-5">
                ...
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Aasaaska Sharciga Ee Qabashada</label>
                <textarea name="legal_basis_for_seizure" rows="2"
                    class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all mb-4">{{ old('legal_basis_for_seizure', $d->legal_basis_for_seizure ?? '') }}</textarea>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Sarkaalka Codsanaya</label>
                        <input type="text" name="requesting_officer"
                            value="{{ old('requesting_officer', $d->requesting_officer ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Lambarka Amarka Maxkamadda</label>
                        <input type="text" name="court_order_reference"
                            value="{{ old('court_order_reference', $d->court_order_reference ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                    <div>
            ...
            <div class="rounded-2xl border-2 border-primary-200 p-5 mb-5" style="background:rgba(82,140,190,0.03)">
                <h3 class="flex items-center gap-2 text-sm font-bold text-neutral-800 mb-4">
                    <i class="bi bi-clipboard-check text-primary-400"></i> Xaalada Qabashada
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Xaalada</label>
                        <select name="seizure_status"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                            @foreach($statusOptions as $opt)
                                <option value="{{ $opt }}" {{ old('seizure_status', $d->seizure_status ?? $statusOptions[0]) === $opt ? 'selected' : '' }}>{{ $opt }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Taariikhda Qabashada</label>
                        <input type="date" name="seizure_date"
                            value="{{ old('seizure_date', $d?->seizure_date?->format('Y-m-d') ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                    <div class="md:col-span-2">
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Goobta Kaydka Hantida</label>
                        <input type="text" name="custody_location"
                            value="{{ old('custody_location', $d->custody_location ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                </div>
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Faallooyinka Awoodda Hantida</label>
                <textarea name="disposition_notes" rows="3"
                    class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">{{ old('disposition_notes', $d->disposition_notes ?? '') }}</textarea>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 p-5 mb-5">
                ...
                @if($d->supporting_document_path ?? null)
                    <a href="{{ asset('storage/' . $d->supporting_document_path) }}" target="_blank"
