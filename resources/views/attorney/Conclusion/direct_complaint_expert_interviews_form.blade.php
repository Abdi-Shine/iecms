...
@section('page_title', 'Wareysiga Khabiirka — ' . $case->case_number)
@section('admin_main_content')

@php
    $d = $case->expertInterview;
    $isComplete = (bool) $d;
...
<div class="p-4 sm:p-6 w-full">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-neutral-100 flex items-center gap-2"
            ...
            <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Wareysiga Khabiirka</span>
        </div>

        <form action="{{ route('attorney-cases.workflow.evidence-interviews.expert-interviews.store', $case->ACID) }}" method="POST"
            ...
            <div class="rounded-2xl border border-neutral-200 p-5 mb-5">
                <h3 class="flex items-center gap-2 text-sm font-bold text-neutral-800 mb-4">
                    <i class="bi bi-mortarboard text-primary-400"></i> Faahfaahinta Khabiirka
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Magaca Khabiirka</label>
                        <input type="text" name="expert_name"
                            value="{{ old('expert_name', $d->expert_name ?? '') }}"
                    ...
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Xirfadda / Takhasuska</label>
                        <input type="text" name="specialization"
                            value="{{ old('specialization', $d->specialization ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                    <div class="md:col-span-2">
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Shahaadooyinka</label>
                        <input type="text" name="credentials"
                            value="{{ old('credentials', $d->credentials ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 p-5 mb-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
                    ...
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Heshiiska Lacagta</label>
                        <input type="text" name="fee_arrangement" placeholder="tusaale: $200"
                            value="{{ old('fee_arrangement', $d->fee_arrangement ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border-2 border-primary-200 p-5 mb-5" style="background:rgba(82,140,190,0.03)">
                <h3 class="flex items-center gap-2 text-sm font-bold text-neutral-800 mb-4">
                    <i class="bi bi-chat-square-text text-primary-400"></i> Ra'yiga Khabiirka
                </h3>
                <textarea name="expert_opinion_summary" rows="4" placeholder="Sharax ra'yiga khabiirka..."
                    class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all mb-4">{{ old('expert_opinion_summary', $d->expert_opinion_summary ?? '') }}</textarea>
                <label class="flex items-center gap-2.5 mb-4 cursor-pointer">
                    <input type="checkbox" name="report_attached" value="1"
                        {{ old('report_attached', $d->report_attached ?? false) ? 'checked' : '' }}
                        class="w-4 h-4 rounded border-neutral-300 text-primary-400 focus:ring-primary-400 cursor-pointer">
                    <span class="text-sm font-semibold text-neutral-700">Warbixin Khabiirnimo Ayaa Lifaaqan</span>
                </label>
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Warbixinta Khabiirka (Fayl)</label>
                <input type="file" name="expert_report"
                ...
                @if($d->expert_report_path ?? null)
                    <a href="{{ asset('storage/' . $d->expert_report_path) }}" target="_blank"
