...
@section('page_title', 'Baaritaan Iyo Qabashada — ' . $case->case_number)
@section('admin_main_content')

@php
    $d = $case->searchAndSeizure;
    $isComplete = (bool) $d;
...
<div class="p-4 sm:p-6 w-full">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-neutral-100 flex items-center gap-2"
            ...
            <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Baaritaan Iyo Qabashada</span>
        </div>

        <form action="{{ route('attorney-cases.workflow.arrest-decision.search-and-seizure.store', $case->ACID) }}" method="POST"
            ...
            <div class="rounded-2xl border border-neutral-200 p-5 mb-5">
                <h3 class="flex items-center gap-2 text-sm font-bold text-neutral-800 mb-4">
                    <i class="bi bi-house-lock text-primary-400"></i> Codsiga Baaritaanka
                </h3>
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Goobta La Baarayo</label>
                <input type="text" name="location_to_search"
                    value="{{ old('location_to_search', $d->location_to_search ?? '') }}"
                ...
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Waxyaabaha La Doonayo</label>
                <textarea name="items_sought" rows="3"
                    class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 focus:outline-none focus:border-primary-400 transition-all mb-4">{{ old('items_sought', $d->items_sought ?? '') }}</textarea>
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Aasaaska Baaritaanka</label>
                <textarea name="grounds_for_search" rows="3"
                    class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 focus:outline-none focus:border-primary-400 transition-all">{{ old('grounds_for_search', $d->grounds_for_search ?? '') }}</textarea>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 p-5 mb-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="applying_officer"
                            value="{{ old('applying_officer', $d->applying_officer ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Xaalada Waaranka</label>
            ...
            <div class="rounded-2xl border-2 border-primary-200 p-5 mb-5" style="background:rgba(82,140,190,0.03)">
                <h3 class="flex items-center gap-2 text-sm font-bold text-neutral-800 mb-4">
                    <i class="bi bi-clipboard-check text-primary-400"></i> Natiijada Baaritaanka
                </h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-4">
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Taariikhda Baaritaanka</label>
                        <input type="date" name="search_conducted_date"
                            value="{{ old('search_conducted_date', $d?->search_conducted_date?->format('Y-m-d') ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                    <div>
                        <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Sarkaalka Fuliyay Baaritaanka</label>
                        <input type="text" name="search_conducted_by"
                            value="{{ old('search_conducted_by', $d->search_conducted_by ?? '') }}"
                            class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all">
                    </div>
                ...
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Waxyaabaha La Qabtay</label>
                <textarea name="items_seized" rows="3" placeholder="Liistee waxyaabaha la qabtay..."
                    class="w-full px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-white focus:outline-none focus:border-primary-400 transition-all mb-4">{{ old('items_seized', $d->items_seized ?? '') }}</textarea>
                <label class="flex items-center gap-2.5 cursor-pointer">
                    <input type="checkbox" name="property_receipt_issued" value="1"
                        {{ old('property_receipt_issued', $d->property_receipt_issued ?? false) ? 'checked' : '' }}
                        class="w-4 h-4 rounded border-neutral-300 text-primary-400 focus:ring-primary-400 cursor-pointer">
                    <span class="text-sm font-semibold text-neutral-700">Rasiidka Hantida Waa La Bixiyay</span>
                </label>
            </div>
            ...
            <div class="rounded-2xl border border-neutral-200 p-5 mb-5">
                ...
                <label class="text-[10px] font-black text-neutral-500 uppercase tracking-wider mb-2 block">Warbixinta Baaritaanka</label>
                <input type="file" name="search_report"
                ...
                @if($d->search_report_path ?? null)
                    <a href="{{ asset('storage/' . $d->search_report_path) }}" target="_blank"
