...
@section('page_title', 'Codsiga Summadda Archiifka — Xukun')
...
@php
    ...
    $kaaliyeName     = $clerkSig?->signer?->EmpName ?? $clerk?->employee?->EmpName ?? $judgment->stamp_requested_by ?? 'Kaaliyaha Maxkamadda';
    ...
    <div class="doc-toolbar-left">
        ...
        <div>
            ...
            <p class="doc-toolbar-sub">
                {{ $case->FileNo }} · Taariikhda Xukunku: {{ $judgmentDate }}
            </p>
        </div>
    </div>
    <div class="doc-toolbar-actions">
        @if(!$myAlreadyActed && $myRole === 'kaaliye')
        ...
        @elseif(!$isApproved && $myRole === 'archive_officer' && $isRequested)
            <button onclick="openSignModal()"
    ...
    <div class="doc-body">
        ...
        <div style="border:1.5px solid #d1d5db;border-radius:8px;padding:12px 16px;margin:14px 0;background:#fafafa">
            ...
            <table style="width:100%;font-size:12.5px;border-collapse:collapse">
                ...
                <tr>
                    <td style="padding:3px 0;color:#6b7280">Taariikhda Xukunta:</td>
                    <td style="padding:3px 0;font-weight:700;color:#111827">{{ $judgmentDate }}</td>
                </tr>
                @if($judgment->judgment_type)
                <tr>
                    <td style="padding:3px 0;color:#6b7280">Nooca Xukunta:</td>
                    <td style="padding:3px 0;font-weight:700;color:#111827">{{ $judgment->judgment_type }}</td>
                </tr>
                @endif
                @if($judgment->judgment_outcome)
                <tr>
                    <td style="padding:3px 0;color:#6b7280">Natiijada Xukunta:</td>
                    <td style="padding:3px 0;font-weight:700;color:#111827">{{ $judgment->judgment_outcome }}</td>
                </tr>
                @endif
            </table>
        </div>
    ...
    <div style="background:white;border-radius:16px;padding:2rem;width:100%;max-width:420px;box-shadow:0 20px 60px rgba(0,0,0,.2);margin:1rem">
        ...
        <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:1.25rem">
            ...
            <div>
                ...
                <p style="font-size:.78rem;color:#6b7280;margin:0">{{ $case->FileNo }} · {{ $judgmentDate }}</p>
            </div>
...
function submitSign() {
    ...
    fetch('{{ route("document.sign", ["type" => "execution_judgment_stamp", "id" => $judgment->id]) }}', {
        method: 'POST',
        headers: {
...
function downloadPDF() {
    ...
    html2pdf()
        .set({
            margin: 0,
            filename: 'JudgmentStampRequest-{{ addslashes($case->FileNo) }}-{{ $docDate }}.pdf',
            image:       { type: 'jpeg', quality: 0.98 },
            html2canvas: { scale: 2, useCORS: true, logging: false, width: 794, height: 1123, scrollX: 0, scrollY: 0 },
