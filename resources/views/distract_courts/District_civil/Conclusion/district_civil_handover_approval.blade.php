...
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-neutral-500 mt-0.5">Dib u eeg, oggolow ama diibi codsiyada dhaqdhaqaaqa dacwadaha
                    madaniga</p>
            </div>
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <a href="{{ route('civil-case-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Sug Qaatay'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
                title="Shaandhee sugayaasha">
            ...
            <a href="{{ route('civil-case-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Qaatay'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
                title="Shaandhee la ogolaaday">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <a href="{{ route('civil-case-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Rejected'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
                title="Shaandhee la diiday">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
            ...
            <a href="{{ route('civil-case-handover.approval', array_merge(request()->except(['status', 'page']), ['status' => 'Draft'])) }}"
                class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4 cursor-pointer"
                title="Shaandhee muswaaadooyinka">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
                <form action="{{ route('civil-case-handover.approval') }}" method="GET"
                    ...
                    <div class="relative flex-1 min-w-[200px]">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            placeholder="Raadi lambarka kiiska, maxkamadda, ama cidda gudbisay..." class="w-full pl-10 pr-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50
                    ...
                    {{-- Case Type filter --}}
                    <select name="casetype" onchange="this.form.submit()" class="px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 text-neutral-600
                        ...
                        @foreach($caseTypes as $ct)
                            <option value="{{ strtolower($ct) }}" {{ request('casetype') == strtolower($ct) ? 'selected' : '' }}>
                                {{ $ct }}</option>
                        @endforeach
                    </select>

                    {{-- Sub Case (Nooca Dacwada) filter --}}
                    <select name="sub_case" onchange="this.form.submit()" class="px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 text-neutral-600
                                       outline-none focus:border-[#528CBE] cursor-pointer transition-all"
                        style="min-width:150px">
                    ...
                    <select name="status" onchange="this.form.submit()" class="px-4 py-2.5 text-sm border border-neutral-200 rounded-xl bg-neutral-50 text-neutral-600
                        ...
                        <option value="all" {{ request('status', 'Sug Qaatay') == 'all' ? 'selected' : '' }}>Dhammaan Xaalado
                        </option>
                        <option value="Sug Qaatay" {{ request('status', 'Sug Qaatay') == 'Sug Qaatay' ? 'selected' : '' }}>Sug
                            Qaatay</option>
                        <option value="Qaatay" {{ request('status', 'Sug Qaatay') == 'Qaatay' ? 'selected' : '' }}>Qaatay
                        </option>
                        <option value="Rejected" {{ request('status', 'Sug Qaatay') == 'Rejected' ? 'selected' : '' }}>La
                            Diiday</option>
                        <option value="Draft" {{ request('status', 'Sug Qaatay') == 'Draft' ? 'selected' : '' }}>Muswaaado
                        </option>
                    </select>
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr style="background:rgba(82,140,190,0.06)">
                            ...
                            <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Dacwada
                            </th>
                            <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Nooca Dacwada
                            </th>
                            <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Cidda Gudbisay
                            </th>
                    ...
                    <tbody class="divide-y divide-neutral-100">
                        @forelse($handovers as $i => $h)
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors" id="row-{{ $h->id }}">
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="text-xs font-bold text-neutral-400">{{ str_pad($handovers->firstItem() + $i, 2, '0', STR_PAD_LEFT) }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    @if($h->case->SubCase ?? null)
                                        <span class="text-xs font-semibold px-2.5 py-1 rounded-full"
                                            style="background:rgba(10,40,77,0.08);color:#0A284D">{{ $h->case->SubCase }}</span>
                                    @else
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        {{-- View --}}
                                        <a href="{{ route('civil-case-handover.document', $h->civil_case_id) }}"
                                            title="Arag Warqadda"
                                        ...
                                        @if($hs === 'Sug Qaatay')
                                            {{-- Approve --}}
                                            <button type="button"
                                                onclick="approveSingle({{ $h->civil_case_id }}, {{ $h->id }}, this)" title="Oggolow"
                                            ...
                                            {{-- Reject --}}
                                            <button type="button"
                                                onclick="rejectSingle({{ $h->civil_case_id }}, {{ $h->id }}, this)" title="Diid"
                                                class="w-8 h-8 flex items-center justify-center rounded-lg border transition-all"
    ...
    <script>
        ...
        async function approveSingle(caseId, rowId, btn) {
            ...
            setLoading(btn, true);
            try {
                const res = await fetch(`/civil-case-handover/${caseId}/approve`, {
                    method: 'POST',
        ...
        async function rejectSingle(caseId, rowId, btn) {
            ...
            setLoading(btn, true);
            try {
                const res = await fetch(`/civil-case-handover/${caseId}/reject`, {
                    method: 'POST',
                    headers: { 'X-CSRF-TOKEN': CSRF, 'Accept': 'application/json', 'Content-Type': 'application/json' },
