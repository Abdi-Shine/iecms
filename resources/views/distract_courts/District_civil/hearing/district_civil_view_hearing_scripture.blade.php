...
@section('admin_main_content')
    ...
    <div class="page-wrap">
        ...
        <div class="data-card">
            ...
            {{-- Search & Filter --}}
            <form action="{{ route('hearings.scripture') }}" method="GET" class="reg-filter">
                {{-- Page size --}}
                ...
                @if(request()->anyFilled(['search', 'status', 'sub_case']))
                    <a href="{{ route('hearings.scripture') }}" class="act-btn" title="Clear" style="width:auto;padding:0 .9rem;gap:.4rem;">
                    ...
                    <tbody>
                        @forelse($records as $i => $r)
                            ...
                            <tr>
                                ...
                                @php
                                    $latestScripture = \App\Models\HearingScripture::where('civil_case_id', $r->CRID)->orderByDesc('created_at')->first();
                                @endphp
                                <td>
                                    <div class="td-actions">
                                        <a href="{{ route('hearings.scripture.create', $r->CRID) }}" title="Ku Dar Garmaqal"
                                            class="act-btn act-btn-poa">
                                            <i class="bi bi-journal-plus"></i>
                                        </a>
                                        @if($latestScripture && $latestScripture->status === 'Draft')
                                            <a href="{{ route('hearings.scripture.edit', $latestScripture->id) }}"
                                        ...
                                        @endif
                                        @if($latestScripture)
                                            <a href="{{ route('hearings.scripture.document', $latestScripture->id) }}"
