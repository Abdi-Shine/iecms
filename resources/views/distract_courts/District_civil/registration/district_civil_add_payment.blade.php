...
@php
    ...
    $partiesForJs = $case
        ? $case->parties->map(function ($p) use ($roleLabels) {
            return [
...
<div class="p-4 sm:p-6 w-full" x-data="paymentRequestForm">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ $case ? route('civil-registration.supporting', $case->CRID) : route('finance.applicant-requests') }}"
    ...
    <form action="{{ route('civil-registration.payment-request.store') }}" method="POST">
        @csrf
        ...
        <div class="rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="bg-white divide-y divide-neutral-100">
                ...
                {{-- Court Service Info (standalone launch only — case-specific requests are filled in later by Finance) --}}
                @if(!$case)
                    <div>
                        <div class="px-6 py-3 flex items-center gap-2 bg-neutral-50/60">
                            <i class="bi bi-bank text-sm" style="color:#528CBE"></i>
                            <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Macluumaadka Adeegyada Maxkamadaha</span>
                        </div>
                        <div class="px-6 py-5">
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                                <div>
                                    <label class="block text-xs font-bold text-neutral-600 mb-1.5">Magaca Maxkamadaha <span class="text-red-500">*</span></label>
                                    <select name="court_id" x-model="courtId" @change="onCourtChange()" required
                                        class="w-full px-3.5 py-2.5 text-sm border border-neutral-200 rounded-lg bg-white cursor-pointer
                                               focus:outline-none focus:border-[#528CBE] focus:ring-2 focus:ring-[#528CBE]/20 transition-all">
                                        <option value="">-- Dooro Maxkamadda --</option>
                                        @foreach($courts as $court)
                                            <option value="{{ $court->CAI }}" {{ old('court_id') == $court->CAI ? 'selected' : '' }}>{{ $court->longName }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-neutral-600 mb-1.5">Nooca Adeega <span class="text-red-500">*</span></label>
                                    <select name="tariff_id" x-model="tariffId" @change="onTariffChange()" required
                                        :disabled="!courtId"
                                        class="w-full px-3.5 py-2.5 text-sm border border-neutral-200 rounded-lg bg-white cursor-pointer
                                               focus:outline-none focus:border-[#528CBE] focus:ring-2 focus:ring-[#528CBE]/20 transition-all disabled:bg-neutral-100 disabled:cursor-not-allowed">
                                        <option value="">-- Dooro Adeega --</option>
                                        <template x-for="t in filteredTariffs" :key="t.id">
                                            <option :value="t.id" x-text="t.name_so"></option>
                                        </template>
                                    </select>
                                    <p class="text-[11px] text-neutral-400 mt-1" x-show="!courtId">Marka hore dooro Maxkamadda.</p>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-neutral-600 mb-1.5">Tarifad Lambar</label>
                                    <input type="text" readonly :value="tariffCode" placeholder="—"
                                        class="w-full px-3.5 py-2.5 text-sm border border-neutral-200 rounded-lg bg-neutral-100 text-neutral-500 font-mono">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-neutral-600 mb-1.5">Cadadka Ajuurada <span class="text-red-500">*</span></label>
                                    <div class="relative">
                                        <span class="absolute left-3.5 top-1/2 -translate-y-1/2 text-neutral-400 text-sm font-semibold">$</span>
                                        <input type="number" step="0.01" min="0" name="amount" x-model="amount" required
                                            class="w-full pl-8 pr-3.5 py-2.5 text-sm border border-neutral-200 rounded-lg bg-white
                                                   focus:outline-none focus:border-[#528CBE] focus:ring-2 focus:ring-[#528CBE]/20 transition-all">
                                    </div>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-neutral-600 mb-1.5">Tariikhda Lacag Bixinta <span class="text-red-500">*</span></label>
                                ...
                                {{-- Live Summary Card --}}
                                <div class="rounded-lg border border-dashed border-neutral-200 bg-neutral-50/60 px-4 py-3 flex flex-col justify-center">
                                    <p class="text-[10px] font-black uppercase tracking-wider text-neutral-400 mb-1">Wadarta Lacagta</p>
                                    <p class="text-xl font-black" :class="amount ? 'text-neutral-800' : 'text-neutral-300'" x-text="'$' + (amount ? Number(amount).toFixed(2) : '0.00')"></p>
                                    <p class="text-[11px] text-neutral-400 mt-0.5" x-text="tariffCode ? tariffCode : 'Weli adeeg lama dooran'"></p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('paymentRequestForm', () => ({
            ...
            get filteredTariffs() {
                ...
                return this.tariffs.filter(t => !t.court_id || String(t.court_id) === String(this.courtId));
            },
            ...
            onTariffChange() {
                const t = this.tariffs.find(t => String(t.id) === String(this.tariffId));
                this.tariffCode = t ? (t.tariff_code || '') : '';
                this.amount = t ? t.amount : '';
            },
