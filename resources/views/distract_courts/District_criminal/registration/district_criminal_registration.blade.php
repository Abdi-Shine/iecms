...
@section('page_title', 'Diiwaanka Ciqaabta — Criminal Registry')
@section('admin_main_content')
    <meta name="csrf-token" content="{{ csrf_token() }}">
    {{-- .ci, .cl, .ci-g, .action-btn and hover variants defined in resources/css/app.css --}}

    <div class="p-4 sm:p-6 w-full" x-data="criminalRegManager">

        {{-- Page Header --}}
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Diiwaanka Ciqaabta</h1>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
            ...
            <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
                <form action="{{ route('criminal-registration.index') }}" method="GET"
            ...
            <div class="px-6 py-4 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Criminal Case Registry</span>
                </div>
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('criminal-registration.supporting', $r->CMID) }}"
                                        ...
                                        <a href="{{ route('criminal-registration.show', $r->CMID) }}" title="Case Information"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                                        ...
                                        <button
                                            @click="openModal({{ $r->CMID }},'{{ addslashes($r->RegisterNo) }}','{{ addslashes($r->FileNo) }}','{{ addslashes($r->GradeCourt) }}','{{ addslashes($r->CaseType) }}','{{ addslashes($r->SubCase ?? '') }}','{{ $r->OpenDate }}','{{ addslashes($r->NumberLetter ?? '') }}','{{ addslashes($r->LegalBasis ?? '') }}','{{ addslashes($r->Orders_Requested ?? '') }}','{{ addslashes($r->Remarks ?? '') }}','{{ addslashes($r->Status) }}')"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                                            onmouseover="this.style.background='#528CBE';this.style.borderColor='#528CBE';this.style.color='white'"
                                            onmouseout="this.style.background='';this.style.borderColor='';this.style.color=''">
                                            <i class="bi bi-pencil-square text-xs"></i>
                                        </button>
                                        <button @click="deleteRecord({{ $r->CMID }})"
                                            class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-20 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-neutral-400 font-medium text-sm">No criminal cases found.</p>
                                        <button @click="openModal()"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 style="color:white;font-size:1.05rem;font-weight:700;margin:0"
                                x-text="isEditMode?'Edit Civil Case':'Diiwaanka Dacwada Ciqaabta'"></h2>
                            ...
                            <div
                                ...
                                <div>
                                    <p style="margin:0;font-size:.82rem;color:#0A284D;font-weight:600">
                                        Lambarka Dacwada Ciqaabta: &nbsp;<span x-text="f.FileNo"
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('criminalRegManager', () => ({
                    isModalOpen: false, isEditMode: false, isSubmitting: false, currentId: null,
                    viewModalOpen: false, v: {},
                    f: { RegisterNo: '', FileNo: '', GradeCourt: '{{ $userCourtID ?? "" }}', CaseType: 'Ciqaabta', SubCase: '', OpenDate: '{{ date("Y-m-d") }}', NumberLetter: '', LegalBasis: 'Dacwadda Ciqaabta', Orders_Requested: '', Remarks: '', Status: 'Diwaan Galin' },
                    init() {
                        this.$watch('f.GradeCourt', (courtcode) => {
                            if (courtcode && !this.isEditMode) this.generateFileNo(courtcode);
                        });
                    },
                    openModal(id = null, RegisterNo = '', FileNo = '', GradeCourt = '{{ $userCourtID ?? "" }}', CaseType = 'Ciqaabta', SubCase = '', OpenDate = '{{ date("Y-m-d") }}', NumberLetter = '', LegalBasis = 'Dacwadda Ciqaabta', Orders_Requested = '', Remarks = '', Status = 'Diwaan Galin') {
                    ...
                    closeModal() { this.isModalOpen = false; this.currentId = null; this.f = { RegisterNo: '', FileNo: '', GradeCourt: '{{ $userCourtID ?? "" }}', CaseType: 'Ciqaabta', SubCase: '', OpenDate: '{{ date("Y-m-d") }}', NumberLetter: '', LegalBasis: 'Dacwadda Ciqaabta', Orders_Requested: '', Remarks: '', Status: 'Diwaan Galin' }; },
                    async generateFileNo(courtcode) {
                        if (!courtcode || this.isEditMode) return;
                        this.f.FileNo = '';
                        try {
                            const res = await fetch(`/criminal-registration/next-fileno/${encodeURIComponent(courtcode)}`, { headers: { 'Accept': 'application/json' } });
                            if (!res.ok) return;
                    ...
                    async submitForm() {
                        ...
                        try {
                            const url = this.isEditMode ? `/criminal-registration/${this.currentId}` : '/criminal-registration';
                    ...
                    deleteRecord(id) {
                        Swal.fire({ title: 'Ma hubtaa?', text: "Ma awoodi doontid inaad dib u celiso!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#DC2626', cancelButtonColor: '#528CBE', confirmButtonText: 'Haa, tirtir!', cancelButtonText: 'Jooji', customClass: { title: 'text-2xl font-bold text-neutral-800', confirmButton: 'px-6 py-2 rounded-lg font-bold text-sm mx-1', cancelButton: 'px-6 py-2 rounded-lg font-bold text-sm mx-1' }, buttonsStyling: true, showClass: { popup: 'animate__animated animate__fadeInDown animate__faster' }, hideClass: { popup: 'animate__animated animate__fadeOutUp animate__faster' } })
                            .then(async r => {
                                if (r.isConfirmed) {
                                    try {
                                        const res = await fetch(`/criminal-registration/${id}`, { method: 'DELETE', headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' } });
                                        const data = await res.json();
                                        if (res.ok && data.success) {
                    ...
                    viewRecord(id) {
                        const record = @json($records).find(r => r.CMID === id);
                        if (record) {
                            this.v = record;
