...
@section('page_title', 'Codsiga Summadda Archiifka — Qaraarka')
...
@php
    ...
    $qaraarDate      = $closeCase->judgment_date
                         ? \Carbon\Carbon::parse($closeCase->judgment_date)->format('d/m/Y')
                         : '—';
    $isRequested     = (bool) $clerkSig;
    ...
    $kaaliyeName     = $clerkSig?->signer?->EmpName ?? $clerk?->employee?->EmpName ?? 'Kaaliyaha Maxkamadda';
...
<div class="no-print doc-toolbar">
    <div class="doc-toolbar-left">
        ...
        <div>
            ...
            <p class="doc-toolbar-sub">
                {{ $case->FileNo }} · Taariikhda Qaraarka: {{ $qaraarDate }}
            </p>
        </div>
    </div>
    <div class="doc-toolbar-actions">
        ...
        @elseif($isArchiveOfficer && $isRequested && !$archiveSig)
        ...
        @elseif(!$isArchiveOfficer && !$clerkSig)
            <button onclick="openSignModal()"
        ...
        <a href="{{ route('execution-close-case.document', $case->ECID) }}" class="btn-doc-back">
...
<div id="approval-doc" class="doc-page-single">
    ...
    <div class="doc-body">
        ...
        <p class="doc-para">
            ...
            Dukuumintiga Qaraarka Dacwadda Madaniga nooca
            <strong>{{ $case->CaseType }}</strong>
            (Summad: <strong>{{ $case->FileNo }}</strong>),
            oo taariikhda qaraarka ahayd <strong>{{ $qaraarDate }}</strong>,
        ...
        <div style="border:1.5px solid #d1d5db;border-radius:8px;padding:12px 16px;margin:14px 0;background:#fafafa">
            ...
            <table style="width:100%;font-size:12.5px;border-collapse:collapse">
                ...
                <tr>
                    <td style="padding:3px 0;color:#6b7280">Taariikhda Qaraarka:</td>
                    <td style="padding:3px 0;font-weight:700;color:#111827">{{ $qaraarDate }}</td>
                </tr>
                @if($closeCase->judgment_type)
                <tr>
                    <td style="padding:3px 0;color:#6b7280">Nooca Qaraarka:</td>
                    <td style="padding:3px 0;font-weight:700;color:#111827">{{ $closeCase->judgment_type }}</td>
                </tr>
                @endif
            </table>
        </div>
        ...
        <p class="doc-para">
            ...
            baaraan-degto codsigan kooban una oggolaato si oggolanaheedu rasmiga uga
        ...
        <div class="sig-row" style="margin-top:44px;display:flex;align-items:flex-end;justify-content:space-between">
            ...
            {{-- Center: Court stamp after full approval --}}
...
@php
    $isRequested = (bool) $clerkSig;
    $modalTitle  = $isArchiveOfficer ? 'Ogolow Codsigu' : 'Codso Summadda';
    $modalRole   = $isArchiveOfficer ? 'archive_officer' : 'kaaliye';
@endphp
<div id="sign-modal"
    ...
    <div style="background:white;border-radius:16px;padding:2rem;width:100%;max-width:420px;box-shadow:0 20px 60px rgba(0,0,0,.2);margin:1rem">
        ...
        <div style="display:flex;align-items:center;gap:.75rem;margin-bottom:1.25rem">
            <div style="width:40px;height:40px;background:{{ $isRequested ? '#10b981' : '#0A284D' }};border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0">
                <i class="bi bi-{{ $isRequested ? 'patch-check' : 'send' }}" style="color:white;font-size:1.1rem"></i>
            </div>
            <div>
                <h3 style="font-size:1rem;font-weight:800;color:#111827;margin:0">{{ $modalTitle }}</h3>
                <p style="font-size:.78rem;color:#6b7280;margin:0">{{ $case->FileNo }} · {{ $qaraarDate }}</p>
            </div>
        ...
        <div style="margin-bottom:1rem">
            ...
            <input type="hidden" id="sign-role" value="{{ $modalRole }}">
        </div>
        ...
        <div style="display:flex;gap:.75rem">
            ...
            <button id="sign-confirm-btn" onclick="submitSign()"
                    style="flex:2;padding:.7rem;font-size:.82rem;font-weight:700;color:white;background:{{ $isRequested ? '#10b981' : '#0A284D' }};border:none;border-radius:8px;cursor:pointer">
                <i class="bi bi-patch-check"></i> Xaqiiji
            </button>
        </div>
    </div>
</div>
