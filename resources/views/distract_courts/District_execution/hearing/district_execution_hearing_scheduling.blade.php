...
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-neutral-500 mt-0.5">
                    @if($hearing)
                        ...
                        <span class="font-semibold text-primary">{{ $hearing->executionCase->FileNo ?? '—' }}</span>
                    @elseif($selectedCase)
                        Mudeynta dacwada ee
            ...
            <a href="{{ route('execution-hearings.index') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-neutral-600 border border-neutral-200
                                      rounded-xl bg-white hover:bg-neutral-50 transition-all shadow-sm">
                <i class="bi bi-arrow-left"></i> Ku noqo Jadwalka
            </a>
        </div>

        <form method="POST" action="{{ $hearing ? route('execution-hearings.update', $hearing->id) : route('execution-hearings.store') }}"
            ...
            <div class="info-card">
                ...
                <div class="card-body">
                    @if(!$selectedCase)
                        <div class="mb-4">
                            ...
                            <div style="position:relative">
                                <select name="execution_case_id" id="f-case" required
                                    ...
                                    @foreach($cases as $c)
                                        <option value="{{ $c->ECID }}" {{ ($hearing && $hearing->execution_case_id == $c->ECID) ? 'selected' : '' }}>
                                            {{ $c->FileNo }} — {{ $c->CaseType }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    @else
                        <input type="hidden" name="execution_case_id" value="{{ $selectedCase->ECID }}">
                    @endif
            ...
            <div class="flex items-center justify-between gap-3 mt-2">
                <a href="{{ route('execution-hearings.index') }}" class="px-6 py-2.5 text-sm font-semibold text-neutral-600 border border-neutral-200
        ...
        <script>
                    ...
                    tr.innerHTML = `
                                    ...
                                    <td class="px-5 py-3 text-center">
                                        <div class="inline-flex items-center gap-1">
                                            <a href="/execution-hearings/${h.id}/edit"
            ...
            function loadHearings(caseId) {
                ...
                fetch(`/execution-hearings/case/${caseId}/json`)
                    .then(r => r.json())
                    .then(renderHearings)
                    .catch(() => { });
            }
            ...
            @elseif(isset($selectedCase))
                loadHearings({{ $selectedCase->ECID }});
            @endif
