...
@section('admin_main_content')
    ...
    <div class="page-wrap">
        ...
        <div class="page-hd">
            <div class="page-hd-left">
                ...
                <div>
                    ...
                    <p class="page-sub">Maamul iyo fulinta dhageysiyada dacwadaha madaniga</p>
                </div>
            </div>
        </div>
        ...
        <div class="data-card">
            ...
            {{-- Search & Filter --}}
            <form action="{{ route('execution-hearings.scripture') }}" method="GET" class="reg-filter">
                {{-- Page size --}}
                ...
                @if(request()->anyFilled(['search', 'status']))
                    <a href="{{ route('execution-hearings.scripture') }}" class="act-btn" title="Clear" style="width:auto;padding:0 .9rem;gap:.4rem;">
                    ...
                    <tbody>
                        @forelse($records as $i => $r)
                            ...
                            <tr>
                                ...
                                @php
                                    $latestScripture = \App\Models\DistrictExecutionHearingScripture::where('execution_case_id', $r->ECID)->orderByDesc('created_at')->first();
                                @endphp
                                <td>
                                    <div class="td-actions">
                                        <a href="{{ route('execution-hearings.scripture.create', $r->ECID) }}" title="Ku Dar Garmaqal"
                                            class="act-btn act-btn-poa">
                                            <i class="bi bi-journal-plus"></i>
                                        </a>
                                        @if($latestScripture && $latestScripture->status === 'Draft')
                                            <a href="{{ route('execution-hearings.scripture.edit', $latestScripture->id) }}"
                                        ...
                                        @endif
                                        @if($latestScripture)
                                            <a href="{{ route('execution-hearings.scripture.document', $latestScripture->id) }}"
                            ...
                            <tr>
                                <td colspan="8">
                                    <div class="empty-state">
                                        ...
                                        <p class="empty-msg">Dacwad fulinta ah lama helin.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
