    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('partiesManager', () => ({
                ...
                async loadParties() {
                    try {
                        const res = await fetch(`/execution-case-parties/case/${this.caseId}`);
                ...
                removeParty(index) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (party.PID) {
                                try {
                                    const res = await fetch(`/execution-case-parties/${party.PID}`, {
                                        method: 'DELETE',
                ...
                async loadLegalReps() {
                    try {
                        const res = await fetch(`/execution-legal-reps/case/${this.caseId}`);
                        const data = await res.json();
                ...
                async setRep() {
                    ...
                    try {
                        ...
                        fd.append('execution_case_id',  this.caseId);
                        ...
                        const res = await fetch('/execution-legal-reps', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: fd
                        });
                ...
                async removeRep(id) {
                    ...
                    try {
                        const res = await fetch(`/execution-legal-reps/${id}`, {
                            method: 'DELETE',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' }
                        });
                ...
                async sendNotifications() {
                    this.isSending = true;
                    try {
                        const res = await fetch(`/execution-case-parties/notify/{{ $case->ECID }}`, {
                            method: 'POST',
                            headers: {
                ...
                async saveParties() {
                    ...
                    try {
                        ...
                        const res = await fetch('/execution-case-parties', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: formData
                        });
