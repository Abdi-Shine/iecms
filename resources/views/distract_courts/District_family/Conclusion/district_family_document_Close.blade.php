...
<div class="no-print doc-toolbar">
    ...
    <div class="doc-toolbar-actions">
        ...
        @if($isStampApproved)
            <a href="{{ route('family-close-case.stamp-request', $case->FCID) }}"
...
function submitSign() {
    ...
    fetch('{{ route("document.sign", ["type" => "family_close_case", "id" => $judgment->id]) }}', {
        method: 'POST',
...
function submitStamp() {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i class="bi bi-hourglass-split"></i> Sugaya...';
    fetch('{{ route("document.sign", ["type" => "family_close_case_stamp", "id" => $closeCase->id]) }}', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content') },
