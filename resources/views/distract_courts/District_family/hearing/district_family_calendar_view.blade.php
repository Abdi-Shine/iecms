...
@section('admin_main_content')
    ...
    <div class="p-4 sm:p-6 w-full">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Mudeymaha Dacwadaha Qoyska </h1>
            </div>
        </div>
                ...
                <table class="w-full text-left text-sm" id="hearings-table">
                    ...
                    <tbody class="divide-y divide-neutral-100" id="table-body">
                        @forelse($hearings as $i => $h)
                            @php
                                $caseStatus = $h->familyCase->Status ?? 'N/A';
                            ...
                            <tr class="hover:bg-neutral-50 transition-colors hearing-row"
                                data-file="{{ strtolower($h->familyCase->FileNo ?? '') }}" data-status="{{ $h->status }}"
                                data-month="{{ $h->hearing_date->format('Y-m') }}"
                                data-purpose="{{ strtolower($h->hearing_purpose ?? '') }}"
                                data-courtroom="{{ strtolower($h->courtroom ?? '') }}"
                                data-subcase="{{ strtolower($h->familyCase->SubCase ?? '') }}">
                                <td class="px-5 py-3.5"><span
                                        class="text-xs font-bold text-neutral-400">{{ str_pad($i + 1, 2, '0', STR_PAD_LEFT) }}</span>
                                </td>
                                <td class="px-5 py-3.5">
                                    <span class="badge-case-type">{{ $h->familyCase->FileNo ?? '—' }}</span>
                                </td>
                                <td class="px-5 py-3.5 text-xs text-neutral-600">{{ $h->familyCase->CaseType ?? '—' }}</td>
                                <td class="px-5 py-3.5">
                                    @if($h->familyCase->SubCase ?? null)
                                        <span class="text-xs font-semibold px-2.5 py-1 rounded-full"
                                            style="background:rgba(10,40,77,0.08);color:#0A284D">{{ $h->familyCase->SubCase }}</span>
                                    @else
                                ...
                                <td class="px-5 py-3.5 text-center">
                                    <a href="{{ route('family-hearings.edit', $h->id) }}" title="Edit Hearing"
