...
@section('admin_main_content')
    ...
    <div class="page-wrap">
        ...
        <div class="data-card">
            ...
            {{-- Search & Filter --}}
            <form action="{{ route('family-hearings.scripture') }}" method="GET" class="reg-filter">
                {{-- Page size --}}
                ...
                @if(request()->anyFilled(['search', 'status']))
                    <a href="{{ route('family-hearings.scripture') }}" class="act-btn" title="Clear" style="width:auto;padding:0 .9rem;gap:.4rem;">
                    ...
                    <tbody>
                        @forelse($records as $i => $r)
                            ...
                            <tr>
                                ...
                                @php
                                    $latestScripture = \App\Models\DistrictFamilyHearingScripture::where('family_case_id', $r->FCID)->orderByDesc('created_at')->first();
                                @endphp
                                <td>
                                    <div class="td-actions">
                                        <a href="{{ route('family-hearings.scripture.create', $r->FCID) }}" title="Ku Dar Garmaqal"
                                            class="act-btn act-btn-poa">
                                            <i class="bi bi-journal-plus"></i>
                                        </a>
                                        @if($latestScripture && $latestScripture->status === 'Draft')
                                            <a href="{{ route('family-hearings.scripture.edit', $latestScripture->id) }}"
                                        ...
                                        @endif
                                        @if($latestScripture)
                                            <a href="{{ route('family-hearings.scripture.document', $latestScripture->id) }}"
