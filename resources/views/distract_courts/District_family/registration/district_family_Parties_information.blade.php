    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('partiesManager', () => ({
                ...
                removeParty(index) {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            if (party.PID) {
                                try {
                                    const res = await fetch(`/family-case-parties/${party.PID}`, {
                                        method: 'DELETE',
                ...
                async loadLegalReps() {
                    try {
                        const res = await fetch(`/family-legal-reps/case/${this.caseId}`);
                        const data = await res.json();
                ...
                async setRep() {
                    ...
                    try {
                        ...
                        const res = await fetch('/family-legal-reps', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: fd
                        });
                ...
                async removeRep(id) {
                    ...
                    try {
                        const res = await fetch(`/family-legal-reps/${id}`, {
                            method: 'DELETE',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' }
                        });
                ...
                async sendNotifications() {
                    this.isSending = true;
                    try {
                        const res = await fetch(`/family-case-parties/notify/{{ $case->FCID }}`, {
                            method: 'POST',
                            headers: {
                ...
                async saveParties() {
                    ...
                    try {
                        ...
                        const res = await fetch('/family-case-parties', {
                            method: 'POST',
                            headers: { 'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content, 'Accept': 'application/json' },
                            body: formData
                        });
