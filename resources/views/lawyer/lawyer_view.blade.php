...
@section('page_title', 'Lawyer Registry')
@section('admin_main_content')

<div x-data="{ showImport: false, importFile: 'No file chosen' }"
    class="p-4 sm:p-6 w-full">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Lawyer Registry</h1>
            <p class="text-sm text-neutral-500 mt-0.5">Manage registered lawyers and legal representatives</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('lawyer.export') }}"
            ...
            <a href="{{ route('lawyer.create') }}"
                ...
                style="background:#528CBE;text-decoration:none">
                <i class="bi bi-person-plus-fill"></i> Register Lawyer
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Total Lawyers</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">{{ $stats['total'] }}</h3>
                <p class="text-xs font-medium mt-0.5" style="color:#528CBE">All registered</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-xs font-medium mt-0.5" style="color:#059669">Currently active</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                style="background:rgba(82,140,190,0.12)">
                <i class="bi bi-gender-male text-xl" style="color:#528CBE"></i>
            </div>
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Male</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">
                    {{ $stats['male'] }}
                </h3>
                <p class="text-xs font-medium mt-0.5" style="color:#528CBE">Male lawyers</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                ...
                <i class="bi bi-gender-female text-xl" style="color:#F0B43C"></i>
            </div>
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Female</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">
                    {{ $stats['female'] }}
                </h3>
                <p class="text-xs font-medium mt-0.5" style="color:#F0B43C">Female lawyers</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
        ...
        <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
            <form action="{{ route('lawyer.index') }}" method="GET"
        ...
        <div class="px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Master Lawyer Registry</span>
            </div>
            <span class="text-xs text-neutral-400 font-medium">{{ $lawyers->total() }} {{ Str::plural('record', $lawyers->total()) }}</span>
        </div>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr style="background:rgba(82,140,190,0.06)">
                        ...
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">License ID</th>
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Full Name</th>
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Gender</th>
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Phone</th>
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Court</th>
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Position</th>
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Grade</th>
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500 text-center">Status</th>
                        <th class="px-5 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500 text-center">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-neutral-100">
                    @forelse($lawyers as $index => $lawyer)
                        <tr class="hover:bg-neutral-50 transition-colors">
                            <td class="px-5 py-4">
                                <span class="text-xs font-bold text-neutral-400">{{ str_pad($lawyers->firstItem() + $index, 2, '0', STR_PAD_LEFT) }}</span>
                            </td>
                            <td class="px-5 py-4 text-neutral-500 text-sm">{{ $lawyer->LID ?? 'â€”' }}</td>
                            <td class="px-5 py-4">
                                <div class="flex items-center gap-3">
                                    ...
                                    @endif
                                    <span class="font-semibold text-neutral-800">{{ $lawyer->LawyerName }}</span>
                                </div>
                            </td>
                            <td class="px-5 py-4">
                            ...
                            <td class="px-5 py-4 text-neutral-600 text-sm">{{ $lawyer->Phone ?? 'â€”' }}</td>
                            <td class="px-5 py-4 text-neutral-600 text-sm font-medium">
                                {{ $lawyer->court->longName ?? $lawyer->CourtID ?? 'â€”' }}
                            </td>
                            <td class=”px-5 py-4”>
                            ...
                            <td class=”px-5 py-4 text-neutral-600 text-sm”>{{ $lawyer->Grade ?? '—' }}</td>
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('lawyer.show', $lawyer->LRID) }}"
                                        class="w-8 h-8 flex items-center justify-center rounded-lg border border-neutral-200 text-neutral-400 transition-all"
                                        onmouseover="this.style.background='#059669';this.style.borderColor='#059669';this.style.color='white'"
                                    ...
                                    <form action="{{ route('lawyer.destroy', $lawyer->LRID) }}" method="POST"
                        ...
                        <tr>
                            <td colspan="10" class="px-6 py-20 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <p class="text-neutral-400 font-medium text-sm">No lawyers found in the registry.</p>
                                    <a href="{{ route('lawyer.create') }}"
                                        class="mt-1 px-4 py-2 text-xs font-semibold text-white rounded-lg transition hover:opacity-90"
                                        style="background:#528CBE;text-decoration:none">Register First Lawyer</a>
                                </div>
                            </td>
                        </tr>
                    @endforelse
    ...
    <div x-show="showImport" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0"
        ...
        style="background:rgba(10,40,77,0.6);backdrop-filter:blur(6px)" @keydown.escape.window="showImport = false">
        ...
        <div class="bg-white flex flex-col overflow-hidden w-full"
            ...
            x-transition:enter-end="opacity-100 scale-100" @click.outside="showImport = false">
            ...
            <div class="flex items-center justify-between flex-shrink-0"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 style="color:white;font-size:1.05rem;font-weight:700;margin:0">Import Lawyers</h2>
                        <p style="color:rgba(255,255,255,.8);font-size:.75rem;margin:0">Upload a CSV file to bulk-add lawyers</p>
                    </div>
                </div>
            ...
            <div style="padding:1.5rem 1.75rem">
                <div class="flex items-start gap-3 mb-5 p-3 rounded-xl"
                    ...
                    <div style="font-size:.78rem;color:#92400e;line-height:1.5">
                        ...
                        <code style="font-size:.72rem;background:rgba(0,0,0,.06);padding:1px 4px;border-radius:3px">LID, LawyerName, Phone, Email, Gender, DOB, POB, CourtID, Position, status</code><br>
                        <span style="color:#b45309">Rows with empty LawyerName will be skipped.</span>
                    </div>
                </div>

                <form action="{{ route('lawyer.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div style="margin-bottom:1.25rem">
                        ...
                        <label for="import_csv"
                            ...
                            onmouseover="this.style.background='rgba(217,119,6,0.09)'"
                            onmouseout="this.style.background='rgba(217,119,6,0.04)'">
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.875rem;border-top:1.5px solid #f3f4f6">
                        ...
                        <button type="submit"
                            ...
                            <i class="bi bi-upload"></i> Import Lawyers
                        </button>
                    </div>
                </form>
            </div>
