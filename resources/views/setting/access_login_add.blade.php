...
@section('page_title', 'Grant System Access')
...
<div class="p-4 sm:p-6 w-full bg-[#F8F9FA] min-h-screen">
    ...
    <div class="mb-4 flex items-center gap-2 text-[11px] font-bold text-gray-400 uppercase tracking-widest">
        <a href="{{ route('employee.access-login') }}" class="hover:text-[#528CBE] transition-colors">Access Management</a>
        <i class="bi bi-chevron-right text-[10px]"></i>
        <span class="text-[#528CBE]">Grant Access</span>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg shadow-gray-200/50 overflow-hidden">
        ...
        {{-- Header --}}
        <div class="px-6 py-4 flex items-center gap-3" style="background:#528CBE">
            <div class="w-9 h-9 bg-white/10 rounded-xl flex items-center justify-center text-white">
                <i class="bi bi-shield-lock text-lg"></i>
            </div>
            <div>
                <h2 class="text-base font-bold text-white leading-tight">Grant System Access</h2>
                <p class="text-[11px] text-white/70">For: <span class="font-bold">{{ $employee->EmpName }}</span></p>
            </div>
        </div>
        ...
        <div class="p-6">
            ...
            <form action="{{ route('employee.access-login.store', $employee->AID) }}" method="POST">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div style="position:relative">
                            <input type="email" name="system_username" value="{{ old('system_username', $employee->email) }}"
                                   required placeholder="username@iecms.gov.so"
                                   style="width:100%;padding:.65rem 2.25rem .65rem .875rem;font-size:.85rem;border:1.5px solid #d1d5db;border-radius:.625rem;background:#fff;color:#111827;outline:none;box-sizing:border-box;transition:border-color .15s,box-shadow .15s @error('system_username') ;border-color:#f87171 @enderror"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Password <span style="color:#ef4444">*</span></label>
                            <div style="position:relative">
                                <input type="password" name="password" id="pwd" required placeholder="Min. 6 chars"
                        ...
                        <div>
                            <label style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Confirm Password <span style="color:#ef4444">*</span></label>
                            <div style="position:relative">
                                <input type="password" name="password_confirmation" id="pwd2" required placeholder="Re-enter"
                                       style="width:100%;padding:.65rem 2.25rem .65rem .875rem;font-size:.85rem;border:1.5px solid #d1d5db;border-radius:.625rem;background:#fff;color:#111827;outline:none;box-sizing:border-box;transition:border-color .15s,box-shadow .15s"
                                       onfocus="this.style.borderColor='#528CBE';this.style.boxShadow='0 0 0 3px rgba(82,140,190,.15)'"
                                       onblur="this.style.borderColor='#d1d5db';this.style.boxShadow='none'">
                ...
                <div class="mt-6 pt-5 border-t border-gray-100 flex items-center justify-between">
                    ...
                    <button type="submit"
                            ...
                            style="background:#528CBE; box-shadow:0 6px 16px rgba(82,140,190,0.3)">
                        <i class="bi bi-shield-check-fill"></i> Grant Access
                    </button>
                </div>
            </form>
        </div>
...
function checkMatch() {
    if (!pwd2.value) { hint.classList.add('hidden'); return; }
