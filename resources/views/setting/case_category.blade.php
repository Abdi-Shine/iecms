...
@section('page_title', 'Case Category Registry')
@section('admin_main_content')

<meta name="csrf-token" content="{{ csrf_token() }}">


<div class="p-4 sm:p-6 w-full" x-data="caseCategoryManager">

    {{-- Page Header --}}
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Case Category Registry</h1>
            <p class="text-sm text-neutral-500 mt-0.5">Manage case classifications, sub-cases and applicable rules</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            ...
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Total Categories</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">{{ count($categories) }}</h3>
                <p class="text-xs font-medium mt-0.5" style="color:#528CBE">Registered categories</p>
            </div>
        </div>

        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:rgba(240,180,60,0.12)">
                <i class="bi bi-diagram-2 text-xl" style="color:#F0B43C"></i>
            </div>
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Sub Cases</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">
                    {{ $categories->filter(fn($c) => !empty($c->sub_case))->count() }}
                </h3>
                <p class="text-xs font-medium mt-0.5" style="color:#F0B43C">With sub-case</p>
            </div>
        </div>

        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:rgba(82,140,190,0.12)">
                <i class="bi bi-book text-xl" style="color:#528CBE"></i>
            </div>
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">With Rules</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">
                    {{ $categories->filter(fn($c) => !empty($c->rule))->count() }}
                </h3>
                <p class="text-xs font-medium mt-0.5" style="color:#528CBE">Rule-based</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
        ...
        <div class="px-6 py-4 flex items-center justify-between border-b border-neutral-100">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Master Case Category Registry</span>
            </div>
            <span class="text-xs text-neutral-400 font-medium">{{ count($categories) }} {{ Str::plural('record', count($categories)) }}</span>
        </div>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr style="background:rgba(82,140,190,0.06)">
                        ...
                        <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Sub Case</th>
                        <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Rule</th>
                ...
                <tbody class="divide-y divide-neutral-100">
                    ...
                    <tr class="hover:bg-neutral-50 transition-colors">
                        ...
                        <td class="px-6 py-4 max-w-[220px]">
                            @if($cat->sub_case)
                                <span class="text-xs font-semibold px-2.5 py-1 rounded-full inline-block"
                                      style="background:rgba(240,180,60,0.12);color:#C07E15" title="{{ $cat->sub_case }}">
                                    {{ Str::limit($cat->sub_case, 40) }}
                                </span>
                            @else
                        ...
                        <td class="px-6 py-4 max-w-[220px]">
                            @if($cat->description)
                                <span class="text-sm text-neutral-600">{{ Str::limit($cat->description, 60) }}</span>
                            @else
                                <span class="text-xs text-neutral-400 italic">—</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button @click="openModal({{ $cat->id }}, '{{ addslashes($cat->case_name) }}', '{{ addslashes($cat->sub_case ?? '') }}', '{{ addslashes($cat->rule ?? '') }}', '{{ addslashes($cat->description ?? '') }}')"
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="text-neutral-400 font-medium text-sm">No case categories found in the registry.</p>
                                <button @click="openModal()"
        ...
        <div class="px-6 py-4 border-t border-neutral-100">
            <p class="text-xs text-neutral-400 font-medium">
                Showing <span class="font-bold text-neutral-600">{{ count($categories) }}</span> {{ Str::plural('record', count($categories)) }}
            </p>
        </div>
    </div>
    ...
    <div x-show="isModalOpen" style="display:none;"
        ...
        <div class="bg-white flex flex-col overflow-hidden w-full"
             style="max-width:620px;max-height:92vh;border-radius:1.25rem;box-shadow:0 25px 60px rgba(0,0,0,.25)"
            ...
            <div class="flex items-center justify-between flex-shrink-0"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 style="color:white;font-size:1.05rem;font-weight:700;margin:0" x-text="isEditMode ? 'Edit Case Category' : 'Register Case Category'"></h2>
            ...
            <div class="overflow-y-auto flex-1" style="padding:1.5rem 1.75rem">
                <form @submit.prevent="submitForm">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.25rem">
                        <div>
                            ...
                            <div style="position:relative">
                                <select x-model="formData.case_name" required
                                    ...
                                    @foreach($caseTypes as $type)
                                        <option value="{{ $type->case_name }}">{{ $type->case_name }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div>
                            <label style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Sub Case</label>
                            <div style="position:relative">
                                <input type="text" x-model="formData.sub_case"
                    ...
                    <div style="margin-bottom:1.25rem">
                        <label style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Rule / Article</label>
                        <div style="position:relative">
                            <input type="text" x-model="formData.rule"
                                   placeholder="e.g. Article 204 — Penal Code"
                                   style="width:100%;padding:.65rem 2.25rem .65rem .875rem;font-size:.85rem;border:1.5px solid #d1d5db;border-radius:.625rem;background:#fff;color:#111827;outline:none;box-sizing:border-box;transition:border-color .15s,box-shadow .15s"
                                   onfocus="this.style.borderColor='#528CBE';this.style.boxShadow='0 0 0 3px rgba(82,140,190,.15)'"
                                   onblur="this.style.borderColor='#d1d5db';this.style.boxShadow='none'">
                            <i class="bi bi-book" style="position:absolute;right:.7rem;top:50%;transform:translateY(-50%);font-size:.8rem;color:#9ca3af;pointer-events:none"></i>
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom:1.5rem">
                        ...
                        <div style="position:relative">
                            <textarea x-model="formData.description" rows="4"
                                      placeholder="Provide a brief description of this case category…"
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;padding-top:1rem;border-top:1.5px solid #f3f4f6">
                        ...
                        <button type="submit" :disabled="isSubmitting"
                            ...
                            <span x-text="isEditMode ? 'Save Changes' : 'Register Category'"></span>
                        </button>
                    </div>
                </form>
            </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('caseCategoryManager', () => ({
            isModalOpen:  false,
            isEditMode:   false,
            isSubmitting: false,
            currentId:    null,
            formData: { case_name: '', sub_case: '', rule: '', description: '' },

            openModal(id = null, case_name = '', sub_case = '', rule = '', description = '') {
                this.isEditMode  = !!id;
                this.currentId   = id;
                this.formData    = { case_name, sub_case, rule, description };
            ...
            async submitForm() {
                ...
                try {
                    const url    = this.isEditMode ? `/case-category/${this.currentId}` : '/case-category';
            ...
            deleteCategory(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        try {
                            const response = await fetch(`/case-category/${id}`, {
                                method: 'DELETE',
                                headers: {
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                                    'Accept': 'application/json'
