...
@section('page_title', 'City Registry')
@section('admin_main_content')

<meta name="csrf-token" content="{{ csrf_token() }}">


<div class="p-4 sm:p-6 w-full" x-data="cityManager">

    {{-- Page Header --}}
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">City Registry</h1>
            <p class="text-sm text-neutral-500 mt-0.5">Manage cities and the region each belongs to</p>
        </div>
        <button @click="openModal()"
            ...
            <i class="bi bi-plus-lg"></i> Register City
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                ...
                <i class="bi bi-buildings-fill text-xl" style="color:#528CBE"></i>
            </div>
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Total Cities</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">{{ $totalCities }}</h3>
                <p class="text-xs font-medium mt-0.5" style="color:#528CBE">Registered cities</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0"
                ...
                <i class="bi bi-map-fill text-xl" style="color:#F0B43C"></i>
            </div>
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Regions Covered</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">
                    {{ $regionsCovered }}
                </h3>
                <p class="text-xs font-medium mt-0.5" style="color:#F0B43C">Distinct regions</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
        ...
        <div class="px-6 pt-5 pb-4 border-b border-neutral-100">
            <form action="{{ route('city.index') }}" method="GET" class="flex flex-wrap gap-3 items-center">
                ...
                <select name="region" onchange="this.form.submit()"
                    ...
                    @foreach($regions as $region)
                        <option value="{{ $region->id }}" {{ (string) request('region') === (string) $region->id ? 'selected' : '' }}>
                            {{ $region->state_name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="px-6 py-4 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Master City Registry</span>
            </div>
            <span class="text-xs text-neutral-400 font-medium">{{ $cities->total() }} {{ Str::plural('record', $cities->total()) }}</span>
        </div>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr style="background:rgba(82,140,190,0.06)">
                        ...
                        <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">City Name</th>
                        <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Region</th>
                    ...
                    <tr class="hover:bg-neutral-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-xs font-bold text-neutral-400">
                                {{ str_pad($cities->firstItem() + $index, 2, '0', STR_PAD_LEFT) }}
                            </span>
                        </td>

                        {{-- City Name --}}
                        <td class="px-6 py-4">
                            <span class="font-semibold text-neutral-800">{{ $city->city_name }}</span>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button @click="openModal({{ $city->id }}, '{{ addslashes($city->city_name) }}', {{ $city->state_region_id }})"
                        ...
                        <td colspan="4" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-3">
                                <div class="w-16 h-16 rounded-full flex items-center justify-center"
                                    ...
                                    <i class="bi bi-buildings text-2xl" style="color:#528CBE"></i>
                                </div>
                                <p class="text-neutral-400 font-medium text-sm">No cities found in the registry.</p>
                                <button @click="openModal()"
            ...
            <div class="flex items-center justify-between flex-shrink-0" :style="isEditMode ? 'background:#F0B43C;padding:1.1rem 1.5rem;border-radius:1.25rem 1.25rem 0 0' : 'background:#528CBE;padding:1.1rem 1.5rem;border-radius:1.25rem 1.25rem 0 0'">
                <div class="flex items-center gap-3">
                    <div style="width:40px;height:40px;background:rgba(255,255,255,0.12);border-radius:.75rem;display:flex;align-items:center;justify-content:center">
                        <i :class="isEditMode ? 'bi bi-pencil-square' : 'bi bi-buildings-fill'" style="color:white;font-size:1.1rem"></i>
                    </div>
                    <div>
                        <h2 style="color:white;font-size:1.05rem;font-weight:700;margin:0" x-text="isEditMode ? 'Edit City' : 'Register City'"></h2>
            ...
            <div class="overflow-y-auto flex-1" style="padding:1.5rem 1.75rem">
                <form @submit.prevent="submitForm">
                    <div style="display:grid;grid-template-columns:1fr;gap:1.25rem;margin-bottom:1.5rem">
                        <div>
                            <label style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">City Name <span style="color:#ef4444">*</span></label>
                            <div style="position:relative">
                                <input type="text" x-model="formData.city_name" required placeholder="e.g. Baidoa"
                                ...
                                <i class="bi bi-buildings" style="position:absolute;right:.7rem;top:50%;transform:translateY(-50%);font-size:.8rem;color:#9ca3af;pointer-events:none"></i>
                            </div>
                        </div>
                        <div>
                            <label style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Region <span style="color:#ef4444">*</span></label>
                            <div style="position:relative">
                                <select x-model="formData.state_region_id" required
                                       style="width:100%;padding:.65rem 2.25rem .65rem .875rem;font-size:.85rem;border:1.5px solid #d1d5db;border-radius:.625rem;background:#fff;color:#111827;outline:none;box-sizing:border-box;transition:border-color .15s,box-shadow .15s;appearance:none"
                                       onfocus="this.style.borderColor='#528CBE';this.style.boxShadow='0 0 0 3px rgba(82,140,190,.15)'"
                                       onblur="this.style.borderColor='#d1d5db';this.style.boxShadow='none'">
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;padding-top:1rem;border-top:1.5px solid #f3f4f6">
                        ...
                        <button type="submit" :disabled="isSubmitting"
                            ...
                            <span x-text="isEditMode ? 'Save Changes' : 'Register City'"></span>
                        </button>
                    </div>
                </form>
            </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('cityManager', () => ({
            ...
            openModal(id = null, city_name = '', state_region_id = '') {
                this.isEditMode = !!id;
                this.currentId = id;
                this.formData.city_name = city_name;
                this.formData.state_region_id = state_region_id ? String(state_region_id) : '';
            ...
            async submitForm() {
                ...
                try {
                    ...
                    if (response.ok && data.success) {
                        ...
                        throw new Error(data.message || 'Validation failed. City might already exist in this region.');
                    }
                } catch (error) {
                    Swal.fire({
                        title: 'Error!',
