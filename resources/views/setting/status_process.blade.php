...
@section('page_title', 'Status Process Registry')
@section('admin_main_content')

<meta name="csrf-token" content="{{ csrf_token() }}">


<div class="p-4 sm:p-6 w-full" x-data="statusProcessManager">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-neutral-800 tracking-tight">Status Process Registry</h1>
            <p class="text-sm text-neutral-500 mt-0.5">Manage case workflow statuses and process definitions</p>
        </div>
        <div class="flex items-center gap-2">
            {{-- Export CSV --}}
            <a href="{{ route('status-process.export') }}"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style="background:rgba(82,140,190,0.12)">
                <i class="bi bi-activity text-xl" style="color:#528CBE"></i>
            </div>
            <div>
                <p class="text-xs font-semibold text-neutral-400 uppercase tracking-widest">Total Statuses</p>
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">{{ count($statuses) }}</h3>
                <p class="text-xs font-medium mt-0.5" style="color:#528CBE">Process definitions</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-neutral-100 flex items-center gap-4">
            ...
            <div>
                ...
                <h3 class="text-3xl font-black text-neutral-800 leading-tight">
                    {{ $statuses->filter(fn($s) => !empty($s->description))->count() }}
                </h3>
                <p class="text-xs font-medium mt-0.5" style="color:#F0B43C">Described statuses</p>
            </div>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-neutral-100 overflow-hidden">
        ...
        <div class="px-6 py-4 flex items-center justify-between border-b border-neutral-100">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs font-black uppercase tracking-[2px] text-neutral-500">Master Status Process Registry</span>
            </div>
            <span class="text-xs text-neutral-400 font-medium">{{ count($statuses) }} {{ Str::plural('record', count($statuses)) }}</span>
        </div>
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr style="background:rgba(82,140,190,0.06)">
                        ...
                        <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500 w-40">Status Code</th>
                        <th class="px-6 py-3 text-xs font-bold uppercase tracking-wider text-neutral-500">Name</th>
                ...
                <tbody class="divide-y divide-neutral-100">
                    ...
                    <tr class="hover:bg-neutral-50 transition-colors">
                        ...
                        <td class="px-6 py-4">
                            <span class="text-sm font-semibold text-neutral-800">{{ $status->name }}</span>
                        </td>
                        <td class="px-6 py-4 max-w-[260px]">
                            @if($status->description)
                                <span class="text-sm text-neutral-600">{{ Str::limit($status->description, 70) }}</span>
                            @else
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button @click="openModal({{ $status->id }}, '{{ addslashes($status->status_code) }}', '{{ addslashes($status->name) }}', '{{ addslashes($status->description ?? '') }}')"
                        ...
                        <td colspan="5" class="px-6 py-20 text-center">
                            <div class="flex flex-col items-center gap-3">
                                <div class="w-16 h-16 rounded-full flex items-center justify-center" style="background:rgba(82,140,190,0.1)">
                                    <i class="bi bi-activity text-2xl" style="color:#528CBE"></i>
                                </div>
                                <p class="text-neutral-400 font-medium text-sm">No status processes found in the registry.</p>
                                <button @click="openModal()"
        ...
        <div class="px-6 py-4 border-t border-neutral-100">
            <p class="text-xs text-neutral-400 font-medium">
                Showing <span class="font-bold text-neutral-600">{{ count($statuses) }}</span> {{ Str::plural('record', count($statuses)) }}
            </p>
        </div>
    </div>
            ...
            <div class="flex items-center justify-between flex-shrink-0"
                ...
                <div class="flex items-center gap-3">
                    <div style="width:40px;height:40px;background:rgba(255,255,255,0.12);border-radius:.75rem;display:flex;align-items:center;justify-content:center">
                        <i :class="isEditMode ? 'bi bi-pencil-square' : 'bi bi-activity'" style="color:white;font-size:1.1rem"></i>
                    </div>
                    <div>
                        <h2 style="color:white;font-size:1.05rem;font-weight:700;margin:0" x-text="isEditMode ? 'Edit Status Process' : 'Register Status Process'"></h2>
            ...
            <div class="overflow-y-auto flex-1" style="padding:1.5rem 1.75rem">
                <form @submit.prevent="submitForm">
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1.5fr;gap:1rem;margin-bottom:1.25rem">
                        <div>
                            <label style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Status Code <span style="color:#ef4444">*</span></label>
                            <div style="position:relative">
                                <input type="text" x-model="formData.status_code" required
                        ...
                        <div>
                            <label style="display:block;font-size:.68rem;font-weight:800;color:#374151;text-transform:uppercase;letter-spacing:.07em;margin-bottom:.45rem">Name <span style="color:#ef4444">*</span></label>
                            <div style="position:relative">
                    ...
                    <div style="margin-bottom:1.5rem">
                        ...
                        <div style="position:relative">
                            <textarea x-model="formData.description" rows="4"
                                      placeholder="Provide a brief description of this status…"
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;padding-top:1rem;border-top:1.5px solid #f3f4f6">
                        ...
                        <button type="submit" :disabled="isSubmitting"
                            ...
                            <span x-text="isEditMode ? 'Save Changes' : 'Register Status'"></span>
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div class="bg-white flex flex-col overflow-hidden w-full"
            ...
            <div class="flex items-center justify-between flex-shrink-0"
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h2 style="color:white;font-size:1.05rem;font-weight:700;margin:0">Import Statuses</h2>
                        <p style="color:rgba(255,255,255,.8);font-size:.75rem;margin:0">Upload a CSV file to bulk-add statuses</p>
                    </div>
                </div>
            ...
            <div style="padding:1.5rem 1.75rem">
                <div class="flex items-start gap-3 mb-5 p-3 rounded-xl" style="background:rgba(217,119,6,0.07);border:1px solid rgba(217,119,6,0.2)">
                    ...
                    <div style="font-size:.78rem;color:#92400e;line-height:1.5">
                        ...
                        <code style="font-size:.72rem;background:rgba(0,0,0,.06);padding:1px 4px;border-radius:3px">status_code, name, description</code><br>
                        <span style="color:#b45309">Duplicate status_code rows will be skipped.</span>
                    </div>
                </div>

                <form action="{{ route('status-process.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div style="margin-bottom:1.25rem">
                        ...
                        <input type="file" id="import_sp_csv" name="csv_file" accept=".csv,.txt" class="sr-only" required
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;padding-top:.875rem;border-top:1.5px solid #f3f4f6">
                        ...
                        <button type="submit"
                            ...
                            <i class="bi bi-upload"></i> Import Statuses
                        </button>
                    </div>
                </form>
            </div>
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('statusProcessManager', () => ({
            isModalOpen:  false,
            isEditMode:   false,
            isSubmitting: false,
            currentId:    null,
            ...
            formData: { status_code: '', name: '', description: '' },

            openModal(id = null, status_code = '', name = '', description = '') {
                this.isEditMode  = !!id;
                this.currentId   = id;
                this.formData    = { status_code, name, description };
            ...
            async submitForm() {
                ...
                try {
                    const url    = this.isEditMode ? `/status-process/${this.currentId}` : '/status-process';
                    ...
                    if (response.ok && data.success) {
                        ...
                        throw new Error(data.message || 'Validation failed. Status code may already exist.');
                    }
                } catch (error) {
                    Swal.fire({
                        title: 'Error!',
            ...
            deleteStatus(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        try {
                            const response = await fetch(`/status-process/${id}`, {
                                method: 'DELETE',
                                headers: {
                                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                                    'Accept': 'application/json'
